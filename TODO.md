...
- **Workspace 数据源绑定**（体量 L，**ADR + 只读 MVP 实现计划已出 2026-07-15，待执行**）｜ADR：`docs/todo/workspace-datasource-binding-adr.md`（三层模型、mock/mcp-resource/local-files 三 kind、凭证全局/用法项目级、ListSources/ReadSource 读取面、readPolicy 仅 ask/deny、威胁模型）；实现计划：`docs/superpowers/plans/2026-07-15-workspace-datasource-readonly-mvp.md`（16 任务：core `sources/` 模块 + 工具面 + desktop 项目配置中心页 + mock 纵切 e2e）｜锚点：`packages/core/src/settings/schema.ts`（profile 子树旁加 sources 数组）、`packages/desktop/src/renderer/credentials/LinkTab.tsx`｜执行后剩余：真实 OAuth provider adapter、Profile 求交接线（resolver `profile?` 参数已留）、写操作、上传文件解析/索引。
