...
> 最近一次核对：2026-07-13（loop nightly r1–r5 已全部合并 main；本次删除已完成的父↔子双向通知 Phase 0、命名收口 PR2–PR6、DriveAgent 可见性收口、session fork 阶段二，并将 Pet Phase 1 标记为已实现）。
...
- **Pet 顶层 agent 主体（用户私人总管/编排大脑）**（体量 L，**已实现**：Pet Phase 1 全 15 step + 左下角可开关小 pet 挂件已完成，loop r5 已合入 main `0c477805`）｜实现规模：81 files / 7637+ lines；实现摘要：侧栏内嵌宽面板 + running dot + L0/L1 toast peek + 左下角挂件。设计稿：`docs/nightly-2026-07-12/pet-top-level-agent-design.md`；桌面 UI 设计：`docs/nightly-2026-07-12/pet-desktop-ui-design.md`。
