...
- **优化实验室（个人先用）**。先做独立 Panel 的手选材料、冻结评测样本、单模型/单个纯文本 Skill、统一预算授权、有限候选及效果/diff/成本报告；P0/P1a 复用现有任务与连接交接先跑出一份文件报告，不改 Host、不做完整面板；P1b 再做面板与实验室内试用；P2 补 Host 通用指令快照、范围化人工采用和回滚。候选不进入生效 Skill/Memory/dream，不自动花费 Token 或推广到其他模型。见 [技术方案与分期验收](docs/todo/optimization-lab-mvp.md)。
