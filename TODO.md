...
- **聊天软件 channel / Chat gateway（含隧道地址回推）**（体量 L，**Phase 1 + 主流 adapters 代码已实现**）｜锚点：`docs/todo/im-gateway-remote-orchestration.md`、`packages/chat/src/chat-gateway.ts`、`packages/desktop/src/main/im-gateway-control-server.ts`｜实现：可单独发布和使用的 `@cjhyy/code-shell-chat`，通用 middleware/runtime 与 CodeShell integration 解耦；12 个 channel（Telegram/Discord/Slack/飞书/钉钉/个人微信 ClawBot/企业微信/Matrix/Mattermost/LINE/WhatsApp/Teams）、扫码登录、逐渠道 target/user 白名单、webhook 验签、`/open /close /status`、桌面端鉴权 loopback RPC、复用 tunnel/pairing/passcode。待做：真实平台凭据 E2E、固定 webhook ingress 部署、launchd/systemd。
