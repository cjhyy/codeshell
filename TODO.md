...
- 🔴 **[MCP/PATH · 根治] GUI 启动早期用 login-shell 注入真实 PATH(抄 codex 快照 / CC fix-path),一处修处处好** — 用户实测:`episodic-memory` stdio MCP(命令 `node ./cli/mcp-server-wrapper.js`)在桌面端报 **"找不到命令"**。**根因(已核实)**:macOS 从 Finder/Dock 启动 GUI 应用时,`process.env.PATH` 被削成系统精简版(`/usr/bin:/bin:/usr/sbin:/sbin`),**不含** `/opt/homebrew/bin`(node 实际在这)。而 `mcp-manager.ts:68 buildStdioEnv` 把 PATH 从 `process.env.PATH` 转发给 stdio 子进程 → 转发的就是坏 PATH → 裸 `node`/`npx` 必然找不到。本机 `which node` 空、但 `/opt/homebrew/bin/node` 存在,正好复现。
  - **两家怎么做的(已核实备忘 `codex_shell_snapshot_path_strategy` / `codex_cc_steering_and_resume_dedup_internals`)**:① **Codex** 在 `~/.codex/shell_snapshots/` 存**登录 shell 环境完整快照**(含真实 PATH),启动 stdio MCP 时继承快照 PATH——**不是把 node 打包进去**(纠正"codex 内嵌 node"的说法);② **CC** 类似,启动早期用 login shell(`$SHELL -lic 'env'` / `fix-path`/`shell-env`)抓真实 PATH 补进 `process.env.PATH`。共同点:**在进程启动早期一次性修好 PATH**,之后 MCP/git/Bash 全自动继承。
  - **codeshell 现状 = 临时法**:目前是**每个 MCP 手动填绝对路径 + `env.PATH`**(如 chrome-devtools 改成 `/opt/homebrew/bin/npx`,见 `codeshell_chrome_devtools_mcp_path_fix`),治标不治本——每个新 MCP 都要单独配。**根治方向**:桌面端 main 进程启动早期,用 login shell 把真实 PATH 补进 `process.env.PATH`(macOS/Linux GUI 专属);修一处,MCP + git 检测 + Bash 工具全部连带修好。与上一条"GUI 没继承 PATH 探不到 git"同源,可合并做。
  - **另可加诊断(见合并记忆 `codeshell-mcp-path-root-fix-direction`)**:spawn ENOENT 时探测 node 是否真的存在,区分「node 未安装(请安装)」vs「node 已装但 PATH 被削(已自动修复)」,别都笼统报"找不到命令"。

- 🟡 **[记忆/dream · 不对称] 会自动打扫的区(dream)小,不能自动打扫的区(user)反而堆最多** — 现状:平时会话记的东西几乎都直接进 **user 区**(要转正、跨会话可靠),但 user 区 **dream 永远碰不到**(`dream-consolidation.ts:184-190` 硬拒非 dream scope 写入,因为后台 dream 无交互式权限后端),只能靠交互会话里逐条批准删。结果**最该被自动清理的完成态 changelog 恰恰落在永不自动清理的区**——dream 系统提示词自己都写着「已完成的修复/任务 changelog 是 clutter 主要来源应归档」,却对 user 区无能为力。本仓库 project user 区一度堆到 ~50 条,dream 区仅个位数。**根治方向(二选一或都做)**:① 让 dream 能对 user 区**提议**清理(列候选清单,用户一次性批准,而非逐条),绕过"后台无权限"限制;② 把 changelog/过程类记忆改成默认存 **dream 区**(可自动归档),user 区只留耐用事实(架构/根因/偏好),从源头堵住完成态 changelog 进 user 区。关联:`codeshell-dream-only-touches-dream-scope-not-user`、`codex-cc-memory-architecture-comparison`(Codex 把记忆当 generated state 隔离,同思路)。另 dream 触发 `shouldAutoDream()` 缺 Codex 的 rate-limit 阈值跳过(配额低时别烧 token 跑 dream),可一并考虑。
