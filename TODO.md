...
- **优化实验室（个人先用）**。计划 A 地基已实施：安全只读 Skill 快照、默认关闭且仅接入 Desktop 的私有能力包、样本校验与不可变冻结。[计划 B（实验引擎）](docs/superpowers/plans/2026-09-26-optimization-lab-engine.md) 已写、待实施；P1a 仍需统一预算控制、有限候选、效果/diff/成本报告及最小授权页，尚无真实实验效果。P1b 再做完整界面与实验室内试用；P2 补模型请求准入、隔离试跑、指令快照、范围化人工采用和回滚。候选不进入生效 Skill/Memory/dream，不自动花费 Token 或推广到其他模型。见 [技术方案与分期验收](docs/todo/optimization-lab-mvp.md)。
