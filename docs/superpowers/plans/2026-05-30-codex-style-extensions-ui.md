...
**Tech Stack:** Electron + React + TypeScript；core 包 `@cjhyy/code-shell-core`；Vitest（仓库测试框架）。
...
Run: `cd packages/desktop && npx vitest run src/renderer/extensions/uninstallTarget.test.ts`
...
Run: `cd packages/desktop && npx tsc --noEmit -p tsconfig.json` → 无新增错误。
