...
>
> > **✅ 推前权威 gate(2026-06-25 收尾·末次跑含全 12 修)**:`bun run build`(core+tui+meta exit 0)· 全包 typecheck 0(core+tui+desktop)· 全包测试 **core 1751 / cdp 27 / tui 69 / desktop 991 全 0 fail**(末次复跑;desktop 随并行工作增至 991)· desktop core symlink 完好 · 工作树净 · 领先 origin **374** · 我的 worktree 全清(另有 `.claude/worktrees/feat+memory-global-scope` 是**你自己的在建工作**=「全局/项目分流+召回生命周期」memory 专项,branch `worktree-feat+memory-global-scope`@659ee239,含 1 处未提交 memory.ts,独立分支不在 push 链——**勿动**)。**这批含本会话 12 个代码修复 + 你/并行的 `722b3642`(面板feature)、`15e0728f`(mobile-cwd 对齐),三方叠加后整体绿、push-ready。** 我的产品代码足迹精确=12 修涉及文件(cdp/driver·scheduler·store·FileRunStore·executor·mcp-manager/tools·builtin/index·context-tools·pty-service·transcript-reader·trusted-device-store·messageMappers/useRemoteApp),无越界。push / A 节交互真机冒烟仍待你。
> > **✅ 修复运行时实证(非仅单测,直接 import 跑真代码)**:全部安全/footgun 类修复逐个跑通确认真生效——`parseSchedule`(`30s/5m/1h`→ms,`0s/0m/0/5mn/""`→throw)·`clampPtyDim`(`NaN/∞/0/-5`→1,`120.7`→120)·`cdp scroll(NaN)`→deltaY 600 finite·`cdp waitForLoad(NaN)`→ok 不挂·`secretHint`(短"ab"→`****` 不泄漏,长→`****alue` 末4位)·`TrustedDeviceStore.authenticate` timingSafeEqual(正确过/错同长拒/**错异长拒不抛**/空拒)·`GenerateImage/Video` pathPolicy 声明已注册(referenceImages/images/image)·`FileRunStore.list`(offset:-1→钳0全列/limit:-1→空页)·`read_file`(limit:-3→空不丢尾)·**executor 数组 path arg**(out-of-ws 元素 →asked:1 走审批门拒,path-traversal 强制端到端真生效)。·`roomHistoryToEvents`(WS 非数组 messages:undefined/null/123/str/类数组 →[] 不抛,合法数组映射)。**12 修除 MCP-signal(结构性,tsc+33测覆盖)外全运行时跑通。** + CLI 端到端 `run "say hi"` 真跑通一轮(续 G)+ **headless 多路径验**:`echo task | run -o json --output-last-message f` → 合法 JSON(`result:PONG/reason:completed/turnCount:1`)+ 文件写 PONG(stdin 读+JSON 格式化+文件输出三路径一次跑通);`-o stream-json` → 18 行合法 NDJSON(session_started/stream_request_start/text_delta… 每行验过)。headless 输出契约(text/json/stream-json/output-last-message/stdin)全跑通。desktop 全量 989/0(含第12修无回归)。**meta 包 publish 面验**:`dist/{index.js,index.d.ts,cli.js}` 全在,`import {Engine,createServer} from "@cjhyy/code-shell"` 正常 re-export(我的 core 模块级新导出未污染公共面),`node dist/cli.js --version`→0.5.0-rc.2,`npx` 入口活。**4 发布包全 build 干净**(core/tui/meta/cdp exit 0,cdp dist 含 waitForLoad NaN 守卫)。**push 卫生**:`bun.lock` 未被本会话改动(早先 bun install 仅恢复 symlink·no changes),381-commit push diff 无 lockfile/dist/.env/node_modules 混入。**测试覆盖审**:12 个 fix commit 中 11 个带测试文件,唯一 0 的 `7e36048b`(MCP signal)是 optional 参数透传,由 tsc + 既有 33 MCP 测覆盖(无裸修)。**lint**:`lint:engine-bypass` 过(new Engine 仍限 protocol 层),eslint 我改的 11 源文件 0 error;2 warning 经 git blame 确认均 pre-existing(executor:20 import@391295ed、transcript-reader:295@a28fc54c),非本会话引入。
>
...
> - **bug-scan 第八批(desktop IPC service 层:settings/plugins/marketplace/skills/dream/mcp-probe/credentials-service)0 候选 — 全干净**:2 个理论点经复核均误报——`formatNetscapeCookies` 的 `Math.floor(expirationDate)` 入参来自 `session.cookies.get()`(Electron 校验过,NaN 不可达,非磁盘可改);`mcp-probe` finally 清理正确(client.close() 关其 transport,`&& !client` 兜未赋值分支,无泄漏)。**8 批 bug-scan 收官,孤立可测 bug 面确认穷尽。**
> - **bug-scan 第七批(mobile 远程 app + mobile-remote 纯逻辑)**:subagent 报 0 高确信,但自评剔除的 `useRemoteApp` room.history.ok `(event.messages ?? []).map` ——**复核认定为真 robustness bug 并修**(`a37d1f76`):messages 来自 WS 不可信 host,`??` 只挡 null/undefined,非数组→`.map` 抛→手机白屏。抽 `roomHistoryToEvents` 守 Array.isArray,+1 测(同审计三轮 mobile 加固一脉)。其余(access-passcode 锁定/pairing 边界/JSON try-catch/reducer)均干净或已守。**这是第 12 个真 bug;7 批扫完,孤立面确已尽。**
> - **bug-scan 第六批(tui-render/turn-loop/compaction/utils 纯逻辑)0 候选 — 全干净**:render-border/measure-text/token-budget/sliceAnsi/streamGroups/sanitize-messages 等 `.repeat()/.padStart()/slice/除法/数组下标/parse` 全已有 `Math.max(0,..)`/`>0` 守卫/try-catch。**至此 6 批 bug-scan 近全仓扫完,后两批 1 中 + 0 → 孤立可测 bug 面已饱和穷尽,不再开新扫。**
> - **bug-scan 第五批(onboarding/settings/parse helpers/renderer 纯逻辑)2 候选**:#1 `transcript-reader` goal_progress `round: Number(d.round ?? 0)`→畸形 transcript 非数字 round 产 NaN ✅**已修**(`5bde0cbd`,Number.isFinite 否则 0,+1 测;cherry-pick 撞 main 测试文件分叉冲突,已解保留两套测试);#2 onboarding `modelDisplayName` 空 variant 尾空格→**cosmetic 判低价值不修**。其余(settings merge/schema/parse-log/format/streamGroups/sliceAnsi)均干净。**收益已明显递减(5 批扫近全仓,本批仅 1 中 1 cosmetic),孤立 bug 面将尽。**
> - **bug-scan 第四批(desktop main/browser-driver/credentials-login/cdp)4 候选**:Bug1 `cdp waitForLoad(NaN)` →`deadline=NaN`→`while(true)` 无穷循环 ✅**已修 HIGH**(`08c31b22`,非有限/非正 timeout 回退默认 10s);Bug3 `cdp scroll(NaN)`→NaN deltaY ✅**同 commit 修**(回退 600+`Math.abs` 顺修负 amount 翻向);Bug4 `ptyResize(NaN)`→`Math.max(1,NaN)=NaN`→node-pty 崩 ✅**已修**(`970e9f02`,抽 `clampPtyDim` 纯helper+3测);Bug2 credentials-login listener「泄漏」→**核验判误报**(`finish()` 调 `handle.close()`→`win.close()` 销毁窗口→webContents+其 listener 随之释放,且每次登录全新 window+partition 不复用,无僵尸)。范围其余(window-state/runs-service/github-skill/keymap/sender/active-guest)均干净。
...
> | `767e373e` | **§4.3 turn-loop maxTurns 上限集成测试**(之前唯一没覆盖的主退出分支) | TDD,+3 测 |
...
> | `b94f3e3e` | **cost-tracker 成本计算测试**(零覆盖→5 例:验 Phase1 定价外移端到端·含 cache 双计费/归一) | TDD,+5 测 |
> | `8727811a` | onboarding provider-detect helper 测试(prefix/baseUrl 匹配·零覆盖→7 例) | TDD,+7 测 |
> | `7d42cefc` | deriveModelPoolKey 等测试(钉旧 modelKey shadowing bug 不回归·零覆盖→5 例) | TDD,+5 测 |
> | (多笔 docs) | assessment 标进度(§4.1/4.3/4.4/4.7/4.8 + Phase1 全四项) | 纯文档 |
>
> **测试覆盖批(2026-06-24)**:Phase1 外移后顺手给它流经的零覆盖路径补测——cost-tracker 成本计算 + onboarding provider-detect/model-pool helper。core 测试 1670→1682。
>
...
>
> **现状**:工作树净(除用户自己的 brainstorm scratch + plan doc)· typecheck 0 · core 1682/desktop 958/tui 69 全绿 · 领先 origin **238** 未 push。
>
...
- Y-2:LAN 模式 `/health` 与 `/mobile/*` SPA 无鉴权下发;LAN WS `auth.device` 路径**无暴力破解锁定**(锁定只覆盖 tunnel 模式 `access-passcode.ts`)。`remote-host-manager.ts`。**评估(2026-06-25):暂不动**——给 `auth.device` 加锁定要改 live auth handler + 复用 `AccessPasscode` 锁定逻辑,且必须真机跑通手机配对全链路才能确认没误锁合法设备(只能用户在场验);默认关 + 熟人 beta 已 ship-as-is(见下「beta 取舍」),贸然改未经真机验的 auth 路径风险 > 收益。开放给更多人前与 secretHash 真 hash 一并做。
...
- Y-4:cookie lease 明文经 `/tmp`(`credentials-service.ts`),文件 `0o600`。**✅ lease 目录 `mkdirSync` mode 已修(commit `b9915a0f`,现 `mkdirSync(LEASE_DIR,{recursive:true,mode:0o700})`,带「audit Y-4」注释;与附A 矩阵一致,2026-06-25 核实)。** 余:明文经 /tmp 本身(非 keychain)同 R-2 留发后。
