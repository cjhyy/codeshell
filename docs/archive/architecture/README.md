...
> **Archive note (2026-07-06):** this reading-order set predates the desktop app becoming the headline product. For the current desktop main/renderer/IPC surface (per-session agent worker, `window.codeshell.*` bridge, panels, browser-automation host, mobile remote), use the current desktop chapter [`../../architecture/10-desktop-and-mobile.md`](../../architecture/10-desktop-and-mobile.md) and the authoritative inventory [`../../architecture/11-feature-inventory.md`](../../architecture/11-feature-inventory.md) (179 capabilities across desktop main / renderer / TUI).
