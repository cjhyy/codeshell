...
证据：[Panel 接口](../panel-apps.md)、[通用进程](../../packages/server/src/panels/process-service.ts)、[通用存储与工作区](../../packages/server/src/panels/runtime-services.ts)、[Web 当前支持范围](../web-panels.md)。Desktop 和 Web 的能力集合不同，不能把安装成功当成全部方法可用。
...
当前 [MediaLibrary](../../packages/desktop/src/main/media/media-library.ts) 的文件身份检查、摘要、原子提交与范围读取可作为通用资源实现的基础；其媒体类型及上层素材准备流程不能整体视为通用资产平台。[MediaJobService](../../packages/desktop/src/main/media/media-jobs.ts) 的状态和取消机制有复用价值，但目前接收 Host 内存中的 `processor.run` 回调；迁移插件时需要独立进程入口，不能只改名为通用 JobService，再把插件回调加载回主程序。

[PanelMediaService](../../packages/desktop/src/main/media/panel-media-service.ts) 仍在主程序注册具体 TTS、场景生成、素材准备等处理器，并决定它们的恢复策略。这些注册、编排和策略也属于迁出范围。新的 Audio8/Qwen 面板桥已自行处理生成进度、取消和手动重试；接口沿用 `media.jobs.*` 命名不表示对应模型任务仍交给 Host 执行。
...
- [现有 sealed file-argument 与进程](../../packages/server/src/panels/process-service.ts)，当前从 Cookie 专用入口授予。
- [媒体分块入口](../../packages/desktop/src/main/media/panel-media-service.ts)、[录制上传的格式约束](../../packages/desktop/src/main/media/media-recording-ingest.ts)。
...
证据：[现有媒体任务服务](../../packages/desktop/src/main/media/media-jobs.ts)、[Guest 撤销进程](../../packages/server/src/panels/process-service.ts)、面板仓库 `apps/video-studio/src/local-voice-bridge.ts` 的 `PANEL_CLOSED`、Video Download 的内存任务和 Quant Lab 历史数据初始化进程。
...
证据：[Web 上下文](../../packages/server/src/panels/runtime.ts)、[Desktop 上下文类型](../../packages/desktop/src/shared/panel-apps.ts)、[Desktop 请求边界](../../packages/desktop/src/main/panel-app-bridge.ts)，面板仓库 `apps/quant-lab/app/modules/panel-host-call-scheduler.mjs` 和 `apps/video-studio/src/local-voice-process.ts`。
...
证据：[进程查找与 stdio](../../packages/server/src/panels/process-service.ts)、面板仓库 `apps/quant-lab/app/modules/history-data-ui.mjs` 的 launcher、`apps/video-studio/src/local-voice-process.ts` 的工具准备协议，以及 `scripts/build-panels.mjs` 的 `nativeEntries`。
...
证据：[具体语音提供者](../../packages/desktop/src/main/media/media-tts-providers.ts)、[场景模板适配器](../../packages/desktop/src/main/media/hyperframes-adapter.ts)、[媒体服务组合](../../packages/desktop/src/main/media/panel-media-service.ts)、[固定提取约束](../../packages/desktop/src/main/media/media-audio-extract.ts)、[工程与转写实现](../../packages/desktop/src/main/media/media-processors.ts)。
