...
| [`worktree-session-isolation-research.md`](worktree-session-isolation-research.md) | 调研稿/设计输入(未动手) | 对标 Claude Code `--worktree` 与 Codex cwd/sandbox/resume 行为,梳理 CodeShell 当前 `EnterWorktree`/`DriveAgent` 能力边界、worktree session resume 风险与后续 workspace mode 落地顺序 |
...
| [`desktop-streaming-markdown-autoscroll-plan.md`](desktop-streaming-markdown-autoscroll-plan.md) | 已实现/归档候选 | 桌面端流式 Markdown 渲染与自动跟随滚动方案。文内记录 worktree 已实现、测试通过;阶段3(活跃代码块分块高亮、桌面/移动滚动统一)仍为可选后续。确认并入 main 后可移 `docs/archive/` |
