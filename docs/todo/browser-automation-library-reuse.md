...
扩展版本现为 **0.2.1**，最低 Chrome **125**，`hello.protocolVersion` 为 **2**。0.2.1 包含临时浏览器句柄的清理修正，保持 0.2.0 的协议兼容性。宿主必须收到协议 2 的 hello 才接受配对。旧扩展的 hello 即使没有请求 ID，升级错误也会保留到后续配对请求与状态中，明确提示更新并在 `chrome://extensions` 重新加载。不会为了兼容旧扩展恢复 raw CDP 通道。
