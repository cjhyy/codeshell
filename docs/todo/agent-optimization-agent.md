...
2026-09-22 首期收敛：见 [优化实验室：个人先用技术方案](optimization-lab-mvp.md)。先做 CodeShell 内置能力包（feature flag 默认关）、手选样本、单模型、单个纯文本 Skill、预算授权与人工采纳；先报告/导出，再接普通任务的范围化版本采用。本文的多策略、多模型、Memory 与自动采用属于后续方向，不是首期交付承诺。
