...
const srv = createServer((req,res)=>{let b="";req.on("data",c=>b+=c);req.on("end",()=>{
  ...
  const send=o=>{const p=JSON.stringify(o);
    if(String(req.headers.accept??"").includes("text/event-stream")){
      res.writeHead(200,{"content-type":"text/event-stream"});res.write(`event: message\ndata: ${p}\n\n`);res.end();}
    else{res.writeHead(200,{"content-type":"application/json"});res.end(p);}};
  if(!m){res.writeHead(202).end();return;}
  if(m.method==="initialize")return send({jsonrpc:"2.0",id:m.id,result:{protocolVersion:"2025-06-18",capabilities:{tools:{}},serverInfo:{name:"cs",version:"1"}}});
  if(m.method==="notifications/initialized"){res.writeHead(202).end();return;}
  if(m.method==="tools/list")return send({jsonrpc:"2.0",id:m.id,result:{tools:[{name:"panel_list",
    description:"List panels. Pass threadId to select which session's panels to read.",
    inputSchema:{type:"object",properties:{threadId:{type:"string",description:"Session/thread id to read panels for"}},required:["threadId"]}}]}});
  if(m.method==="tools/call"){
    calls.push({args:m.params?.arguments, metaThread:m.params?._meta?.threadId,
...
srv.listen(0,"127.0.0.1",async()=>{
  ...
  const c=spawn("codex",["exec","--skip-git-repo-check","--dangerously-bypass-approvals-and-sandbox",
    "-c",`mcp_servers.cs.url="${url}"`,"-c",`mcp_servers.cs.bearer_token_env_var="T"`,
  ...
  await new Promise(r=>{c.on("close",r);setTimeout(()=>c.kill(),140000);});
