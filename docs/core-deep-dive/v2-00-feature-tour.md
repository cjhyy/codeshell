# CodeShell 架构解析 · 开篇：一个让 Codex 挂机榨干额度的 Agent Harness 长什么样

> 系列定位：不讲“怎么用某个 AI 工具”，而是把一个真实的、正在开发的通用 Agent 编排框架 **CodeShell** 拆开，讲清楚一个能被手机遥控、能在后台无人值守跑长任务的 Agent，它的“运行壳”到底是怎么搭出来的。
>
...
- [一、先说一个很多人都中招的浪费](#一先说一个很多人都中招的浪费)
...
你可能也有过这个瞬间：订了 Codex Pro / Claude Max 这类订阅，额度按 `5h` / `7d` 这样的**滚动窗口**刷新。窗口快到刷新点了，一看——还剩一大半没用完。

不是不想用，是**人的注意力不按滚动窗口工作**。白天开会、通勤、吃饭；晚上也不可能为了“别浪费额度”硬找任务来烧。于是你为之付过钱的那段算力，就在刷新点静悄悄清零了。

| 😩 现象 | 💡 真实原因 | 🔧 CodeShell 的思路 |
|--------|-----------|-------------------|
| 窗口快结束还剩一大半 | 额度按时间窗刷新，人却不能一直盯着 | 让 agent 在你不看屏幕时持续干活 |
| “下个周期一定用满” | 任务不缺，缺的是“有人发起并盯着” | 远程发起 + 后台推进 + 完成通知 |
| 长任务跑一半人走了 | 交互式工具必须人在场 | 长任务可挂机、可续跑、可唤醒 |

🎯 **本篇结论先行**：CodeShell 想解决的不是“模型不够聪明”，而是这个很务实的浪费——把那些你本来就想丢给 Codex / Claude Code 跑的长活，从「必须人在电脑前盯着」变成「可以远程发起、后台推进、完成后通知」。

它不承诺“一定全部榨干”，也不鼓励无脑烧 token；它只是让那些**本来会作废的额度有机会变成真正完成的任务**。
