...
源码主战场:`packages/core/src/protocol/`、`packages/core/src/session/`、`packages/core/src/run/`、`packages/core/src/automation/`、`packages/core/src/arena/`、`packages/core/src/cc-orchestrator/`,以及宿主侧的 `packages/tui/src/cli/`、`packages/desktop/src/main/`。
...
## 八、平台扩展:Plugins、Arena、DriveAgent

当 MVP 和 Production 都稳定后,harness 会自然走向平台化。CodeShell 用三类扩展把 core 变成可扩展平台,而它们都**接入统一的工具/权限管线,不绕过 harness**。

![平台扩展:Arena 与 DriveAgent](assets/v2/v2-05-platform-extensions-arena-driveagent.png)
...
Arena(`arena/`)跑多个模型,两种模式,记一句话就够:**用 Arena 来评审,用 IterativeArena 来创作。**

- **评审模式**(`arena/arena.ts`):证据驱动的 claim 管线——Plan → Evidence → Strategy/Tools → Research → Claims → 辩论/裁决 → Consensus。claim 生命周期是 append-only 状态机(`proposed → under_review → {verified | contested | rejected | unresolved}`),`ArenaLedger` 是带索引的 append-only 存储。Lenses(工程/产品/架构/通用)用角色+标准让同一套机器对准不同关注点。
- **创作模式**(`arena/iterate/iterative-arena.ts`):锦标赛产出 v1,然后 argue/revise 轮并行跑批评者,查收敛,作者轮转,默认最多 5 轮。

关键不变量:append-only ledger、verify/debate 阶段用只读工具包、`AbortSignal` 穿过每个异步阶段。

> **设计取舍:Arena 为什么把"评审"和"创作"拆成两套机器,而不是一个"多模型一起干"的通用循环?** 因为这两件事的成功标准相反。评审要的是**收敛到可信结论**——所以它做成证据驱动的 append-only claim 状态机:每条主张必须有证据、经辩论、被裁决,verify/debate 阶段只给只读工具(评审者不该改代码),ledger 只追加不改写(保留完整的判断链可审计)。创作要的是**发散再择优**——所以它做成锦标赛 + 多轮 argue/revise,允许批评者并行开火、作者轮转、最多 5 轮后收。把它们硬塞进一个循环,会得到一个既不敢下结论又不敢放手发散的四不像。两种模式共享的那几条不变量(append-only、只读评审工具包、`AbortSignal` 穿透每个异步阶段)恰恰是底线:无论评审还是创作,中间状态都不能被偷偷改写,用户一个 Stop 都要能干净地穿过所有并行阶段把它们一起取消——**多模型并发最容易漏的就是取消信号没传到每一路**。
...
12. `arena/arena.ts` + `cc-orchestrator/external-agent-driver.ts` —— 看多模型对垒与外部 CLI 黑盒驱动的非 detached spawn。
...
- **本篇** 是骨架与四肢——协议接缝让一个 core 服务多种宿主,RunManager/cron/goal 撑起无人值守的长任务,plugins/Arena/DriveAgent 把它扩成平台。
