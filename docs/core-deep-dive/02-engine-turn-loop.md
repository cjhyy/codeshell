...
- **Tier 1 — microcompact**:零成本、无损。对可重取的工具(Read/Glob/Grep/Bash/WebFetch 等)清掉较老的 `tool_result` **内容**,留一行指纹 `[Old tool result cleared — ...]`。带状态的工具(TodoWrite/Agent/Skill)原样保留。
