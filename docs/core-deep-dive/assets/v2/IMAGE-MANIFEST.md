...
| --- | --- | --- | --- | --- |
| `v2-01-llm-call-to-harness.png` | `01-core-as-agent-harness.md` | 解释从单次 LLM call 到 research loop 再到 agent harness 的演进。 | Three stages: LLM Call, Research Loop, Agent Harness; model/tools/context/permissions/transcript/host protocol. | 开篇提出“LLM call 不是 agent”之后。 |
...
| `v2-01-presets-not-hardcoding.png` | `01-core-as-agent-harness.md` | 说明 coding 行为来自 preset 配置，不是 core 硬编码。 | Generic Core connected to general and terminal-coding configuration cartridges. | “为什么不是 coding agent / 行为即配置”小节。 |
