...
> **历史快照**：本文中的 worktree、未 commit、merge 顺序和行号只描述 2026-07-09 当时状态；相关修复早已进入主线，不是当前施工清单。当前架构与开放问题分别以 `docs/architecture/` 和根 `TODO.md` 为准。
...
17 号执行计划中的 6 个批次当时已在隔离 worktree TDD 落地，随后全部合入主线。`18`、`19`、`20` 中的分支名、merge 顺序和清理命令仅供追溯，不应在当前工作树重复执行。
