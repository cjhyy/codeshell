...
| **跨切**:外部输入 throw 面(`new RegExp`/`decodeURIComponent`/`new URL`) | RegExp 3 处全 try/catch(permission/path-policy/loadPluginHooks);**decodeURIComponent readCookie 是真 DoS(已修 d2f5e449)**;new URL 外部输入全守卫(web-fetch/credentials-login/policy fail-closed/parseMarketplace reject/github 友好错/automation-host)·oauth req.url 实际不可抛(Node 保证)。除 readCookie 外无未守卫 throw。 |
