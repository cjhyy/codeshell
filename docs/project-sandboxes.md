...
独立 Link 已完成两个真实 Docker 项目的受控上游验收：390px 浏览器授权与原项目
回调、跨项目拒绝、容器内只读动作、重启后保留连接、断开后远端撤销。窄屏顶部持续
显示当前项目及执行位置。可重复的测试、私密配置和 TLS 测试证书边界见
[实际 Docker Link 验收](remote-link-host.md#实际-docker-项目验收)；它不替代真实账号、
物理手机或公网部署验收。
