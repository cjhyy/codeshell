...
assert.ok(["web", "electron", "paired"].includes(mode), "Expected web, electron or paired mode");
...
try {
  ...
  for (const width of widths) {
    ...
    console.log("Link verifier: waiting for Link login/consent", mode, new URL(page.url()).origin);
    if (mode !== "web") {
    ...
    if (mode === "paired") {
      ...
      await page.getByRole("button", { name: "前往 Link 授权", exact: true }).click();
      await page.getByRole("button", { name: "允许只读访问", exact: true }).waitFor();
      const devices = await localWindow.evaluate(() => window.codeshell.mobileRemote.listDevices());
