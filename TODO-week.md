...
- [ ] 发送给模型 / 工具时携带 path 元数据。**已定(2026-06-06):携带 path**——assistant 后续可引用「你刚才那张 /xx.png」、工具能定位原文件;当前模型不支持图片时也至少留 path 可引用。
- [ ] 粘贴、拖拽、文件选择三条入口行为一致。
...
- [ ] 菜单项。**已定(2026-06-06):走 A 固定菜单项**(对齐 Codex,不做 Launch Services 动态枚举)——系统默认 app / 在文件夹中打开 / Cursor(可配编辑器)。用现有 Electron shell API(openPath / showItemInFolder 已接好);Cursor 走 editor 命令。
...
- [ ] outputFile：每个 background agent 写 `~/.code-shell/agents/<agentId>.txt`。**已定(2026-06-06):补 outputFile**——现架构走 notificationQueue 回灌外,额外把每个后台 agent 的输出落盘到 `~/.code-shell/agents/<agentId>.txt`,供外部 tail / 跨 session 查历史;需配套清理(app 退出 / 删 session)。notificationQueue 回灌仍是主路径,outputFile 是外部可读副本。
...
- [ ] Agent 执行结果汇总视图。
- [ ] `AgentStatus` 支持按当前 session 过滤 / 默认只列当前 session 的后台 agent，避免多会话时查到全局进程里所有子 agent。
