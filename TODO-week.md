...
- [~] 运行中续轮 / 加预算(UI 交互层,需 desktop 配合,排后)。**搁置(逛街批)**:UI 交互活,core 侧 maxTurns/budget 已可配。
...
- [~] 配置：memories.maxCount ✅(schema+orchestrator 透传到 parseExtractionResponse,测试 extract-memories.test.ts);**extractionModel 已接**(resolveExtractionClient:extractionModel 命名有效 pool model 则用它跑提取,否则回退 aux→primary;build 失败也回退);**maxAge 仍留位**(MemoryEntry 无时间戳字段,按 age 过滤需先给 entry 加 mtime/createdAt,较大改动,待定)。
