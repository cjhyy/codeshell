...
- [~] 运行中续轮 / 加预算(UI 交互层,需 desktop 配合,排后)。**core+协议+preload 全通,差 renderer 按钮**:
  ...
  - 协议:`agent/goalExtend` method + handleGoalExtend(按 sessionId 路由,无活跃 run 报错)+ ChatSession.extendGoalRun 转发。
  - preload:`window.codeshell.goalExtend(sessionId,{addTurns?,addTokenBudget?,addTimeBudgetMs?})` + 类型。
  - **剩 renderer 按钮**:正确时机是「运行中接近 turn 上限」显示「再续 N 轮」(非 exhausted 后——那时 run 已停,extend 返 null)。需 renderer 拿 live turnCount/maxTurns;留 UI sprint。测试 goal.test.ts applyGoalExtension(6 例)。
