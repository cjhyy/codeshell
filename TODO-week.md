...
- [x] 文件模块对话框支持直接拖入文件，方便从 Finder / 文件面板快速添加。✅ 文件面板图片行加「📎 添加到输入框」hover 按钮(在 ⋯ 打开方式旁):点击→读取 data URL(images:readDataUrl)→`buildPathAttachment` 暂存,**name=绝对路径**(chip 显示真路径 + wire `name` 带真路径,assistant/工具可定位原文件——这是唯一能拿到真绝对路径的入口)。链路 App.attachImageByPath→PanelArea/PanelBody→FilesPanel/DirNode。`buildPathAttachment` 复用 buildAttachments 的 count/type/size 限制(size 从 base64 估)。测试 attachments.test.ts(5 例:暂存/非图拒绝/坏 dataURL/超量/wire 带路径)。
