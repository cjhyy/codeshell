...
- [ ] 图片除了能在消息 / 输入框 / tool result 中预览，也要支持点击放大查看和下载按钮。
- [ ] 做完整 lightbox：原图查看、关闭按钮视觉居中、下载、复制路径、在文件夹中显示、切换下一张 / 上一张，并保留清晰的文件路径 / 来源信息。
...
- [~] outputFile：每个 background agent 写 `~/.code-shell/agents/<agentId>.txt`。**已被超越**：现架构结果只活在 notificationQueue + 用户消息(registry 保持 metadata-only,见 agent-notifications.ts 注释),无 outputFile;若要外部可读再议。
- [~] 删除或替换 AgentStatus 轮询路径。**部分**:schema description 已劝退轮询;AgentStatus 作为可选 companion 保留(查单个/列全部),未删。是否硬删需产品决策。
...
- [x] 命令模式匹配：如允许 `bun test`、`bun run build`。✅ 机制已就位:PermissionClassifier.matchesRule→ruleMatches 按 argsPattern.command 正则匹配;buildProjectRule 把 Bash 收窄到 head 命令(`bun`→放行所有 `bun ...`)。注:当前粒度是 head 命令而非子命令(`bun test` vs `bun`),子命令粒度是可选增强(需产品决定要不要更细)。
...
- [ ] settings schema：`imageGen.providers[]` / `videoGen.providers[]`，各带 default provider。**待定**:当前复用现有 providers[].kind(openai/google)解析,够用;独立 imageGen/videoGen schema 是否需要看 UI 方向,与连接页一起定。
