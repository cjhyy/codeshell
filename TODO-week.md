...
- [ ] **聊天会话标题过长**（如 `s-mpxsdekd-7aab0c06` 这类原始 sessionId 直接当标题）—— 标题应是 LLM 生成的一句话摘要（见记忆 [[project_session_title_llm]]），但仍有会话回退/显示成原始 `s-xxxx-xxxx` id。需查：① 哪些会话没拿到 `session_title` 事件（首轮失败/历史会话/automation 会话），② 侧边栏 fallback 文案从 sessionId 改为更友好的占位（如「新对话」+ 时间）或截断显示。落点：`session_title` 生成链（首条 user 取 transcript）、`Sidebar.tsx` 标题渲染、`SessionSummary` title 字段。
...
- [ ] **view_image 收尾(最终 code-review 非阻塞项 + 范围外增量)** ——
  - 🟡 **M1**:`view-image.ts` vision gate 是 `if (ctx?.llmConfig){...}`,缺失时不读 gate 直接读图。运行时 `ToolContext.llmConfig` 必有(`context.ts:116` 必填),只测试可触发;低风险。**建议**:缺失即当非视觉(对齐 `DEFAULT_CAPABILITY.supportsVision=false` 的保守姿态),防御性默认。
  - 🟠 **gpt-4o 等被误判为非视觉**(review 顺带暴露的**独立**问题,影响面超出 view_image):`capabilitiesFor` 规则里 `gpt-4o` 无匹配规则→落 `DEFAULT_CAPABILITY`(supportsVision:false),著名 vision 模型却被当看不了图。后果:对 gpt-4o 会剥历史图 / Engine.run 拒新图附件。**修**:给 `rules.ts` 补 gpt-4o / gpt-4o-mini / gpt-4-turbo 等的 vision 规则。(本轮 openai 测试已临时改用 `gpt-5` 绕开。)落点 `packages/core/src/llm/capabilities/rules.ts`。
  ...
  - 🟢 **策略 B「看过一轮后把历史图降级成文字摘要」**(计划排除,后续增量):当前靠三道闸门 + 上面的 C1 递归剥离控制污染,够用;主动降级(图只在生成/确认那一两轮带,之后换文字占位)是更激进的省 token 手段,留待需要时做。
  - 🟢 **M2**(review 已确认无害,仅记录):OpenAI 多 tool_result 批次 + view_image image 时,所有 `role:"tool"` 先 emit、image 合并到其后一条 user 消息——wire 合法,sibling textParts 会并入该 image turn(tool-result 批次无 loose user text,benign)。无需改,留察。
