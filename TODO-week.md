...
| 8   | **bug:模型切换后新 session 仍用旧模型**                       | IPC 已通(`onModelChange` 已发 `configure({ model })`,`handleConfigure` 调 `engine.switchModel`),**但** `agent-server-stdio.ts:77` 的 `resolvedLlmConfig` 在 bootstrap 固化,line 114 的 `engineFactory` 给新 session 都用这个 frozen 值。修法:`engineFactory` 改成 `llm: runtime.modelPool.resolveLLMConfig() ?? resolvedLlmConfig`。**现场参考**:session `s-mppnqswb-c71ac80a` 16:46 切到 `deepseek-v4-flash` 后仍持续走 `gpt-5.5` 报 `Connection error.`(同时期 `gpt-5.5` provider 不可达),验证 fix 时可拿这个场景复现 |
