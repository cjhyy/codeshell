...
- [ ] 每个 turn 后面的“编辑 / 文件变更”卡片点审查时，默认只审查该 turn 产生的文件 / diff，而不是直接落到整个工作区未提交 diff。
- [ ] 审查入口支持切换范围：本 turn、未暂存、已暂存、提交、分支、上轮对话。
- [ ] 审查面板文件树按范围展示变更文件，区分 unstaged / staged / committed / branch，并显示增删行数。
