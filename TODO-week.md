...
- **泛化推理强度配置 P0+P1+P2**(`249fddd`→`729ca5b`):富结构 `ReasoningSetting`(off/on/effort/budget)+ `reasoningControlFor` 描述符 + 全链路 thinking→reasoning 改名、去 medium 硬编码;**修了 Anthropic thinking 完全没实现的真 bug**;ModelSection 按 kind 渲染控件。**剩 P3(可选,已 defer)**:OpenRouter max_tokens/enabled 透传、Mistral magistral effort 选项细化。
