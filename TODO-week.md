...
- [x] 文件模块对话框支持直接拖入文件，方便从 Finder / 文件面板快速添加。✅ **两条路**:① 点击「📎 添加到输入框」hover 按钮;② **真拖拽**:图片行 `draggable`,dragstart 写绝对路径到自定义 MIME `application/x-codeshell-path`;composer 的 drop 处理识别该 MIME→onAttachImagePath→暂存。两路都走 `buildPathAttachment`,**name=绝对路径**(chip+wire 带真路径,可定位原文件)。顺修:文件夹行 `draggable={false}`、非图行不可拖(此前文件夹/文本能被浏览器默认拖动、且拖了没用)。链路 App.attachImageByPath→ChatView.onAttachImagePath / PanelArea→FilesPanel。`buildPathAttachment` 复用 count/type/size 限制(size 从 base64 估)。测试 attachments.test.ts(5 例)。
