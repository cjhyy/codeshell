...
- [~] 审查入口支持切换范围：本 turn、未暂存、已暂存、提交、分支、上轮对话。**已做前 4 个**:本轮改动 / 未暂存 / 已暂存 / 全部未提交(范围选择器,porcelain XY 分流 staged/unstaged,无新 IPC)。**committed / branch / 上轮对话留 Slice 2**(需 git log / range diff)。
- [~] 审查面板文件树按范围展示变更文件，区分 unstaged / staged。**已做**:按 scope 过滤展示,porcelain code 区分 staged(X)/unstaged(Y);**增删行数徽章留 Slice 2**(getGitStatus 不带行数,需 diff-stat)。
- [ ] 提供“提交或推送”“创建拉取请求”等顶部动作，但按钮状态要按当前范围和 git 状态禁用 / 启用。
...
- [x] 当前缺口(原 bug):turn 卡片“审查”落到整工作区 + 看不了历史轮对比。✅ 两者均已修(默认本轮范围 + 快照查看)。剩 committed/branch/上轮对话范围切换属 Slice 2。
- [ ] **文件树 + 顶部动作整体对齐截图的审查工作流**:左侧按范围分组的变更文件树(unstaged/staged/committed/branch,带增删行数徽章),顶部一排范围切换 + 提交/推送/建 PR 动作条。这是 2.3a 的"完整形态",非局部修补。
...
- [~] 路径策略 block 时展示原因，并允许批准本次 / 本会话 / 特定路径。**部分**:普通工具审批已支持 本次/本会话/本项目 范围(f5f57ac,ApprovalCard split-button,scope 透传到 InteractiveApprovalBackend);「特定路径」前缀粒度仍缺(需扩 buildProjectRule 非 Bash 工具),与上面路径级规则一起做。
...
> - **6.3 Workspace 数据源**:全新大子系统(资源模型/外部数据源 link/scope 分配/工具 scope 检查/管理 UI),多天活,且与第 1 节 Workspace 升级强耦合——用户已明确第 1 节留着一起做。
> - **6.4 远程控制/跨代理编排**:全新大子系统(SSH/扫码配对/远控会话/编排 Codex+CC/安全边界),多天活,需大量产品决策与真机验证。
