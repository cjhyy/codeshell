...
> 核实(2026-06-06):前 3 项早已实现;后 3 项是**被现架构超越的旧计划**,不应照抄实现。
> 现架构(见 `agent-notifications.ts` 注释)走 notificationQueue 注入,而非 outputFile;
...
- [ ] 同步 `Agent(...)` 超过 120s 自动转后台。**未做(真缺口)**:当前同步 Agent 跑久会占满 30min 工具超时;auto-background 启发式有价值但属新行为,需先和你确认触发阈值/语义再做(类似后台 shell 的 D3 决策)。
