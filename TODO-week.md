...
- [x] **聊天会话标题过长(显示原始 sessionId)** —— **核实为误报(2026-06-03 验证,代码无此路径)**:`SessionSummary.title` 是必填 `string`,创建时 `title?.trim() ? title.trim() : "新对话"`(`transcripts.ts:244`)——缺标题回退「新对话」,**绝不**回退 sessionId;`Sidebar.tsx:556` 直接渲 `s.title` 无 `s.id` fallback;`buildSessionTitle` LLM 失败返回 `null` → 不 emit `session_title` 事件 → 保持「新对话」。原始 `s-xxxx-xxxx` 无法泄漏成标题。关单。
...
- [x] **view_image 工具(对照 Codex,让模型用 vision「看」自己生成的图)** —— **已做(本轮,9 commit `97ca194`→`87d9b3a`,在 main 未 push)**。**做了**:新内建工具 `view_image({path})` 读本地图 → base64 image 块回传进上下文;三道闸门防污染(① vision gate:模型不支持视觉→不读文件只回文字占位;② 格式 gate:仅 png/jpeg/gif/webp,svg 等回「先转 PNG」;③ 大小 gate:>5MB 不读 base64)。**全链路**:`ToolResult.contentBlocks` 新字段 → `BuiltinToolFn` 返回类型扩成 `string|{contentBlocks,result?}` → registry 透传 → turn-loop 抽 `toolResultToBlock` 纯函数转 tool_result 块 → 两路 provider(Anthropic 直接映进 tool_result.content 数组;**OpenAI wire 不许 tool 消息带 image** → 把 image 拆出提升成独立 user `image_url` 消息)。transcript 只存文字镜像不存 base64(存档不膨胀)。注册为 `permissionDefault:"allow"+isReadOnly:true`(同 Read 工具)。**顺修真 bug(最终 review 抓的 Critical)**:`stripVisionFromHistory` 原只看顶层块,不递归 `tool_result.content`→vision 模型 view_image 后切非视觉模型,嵌套 base64 漏发/OpenAI 400;已让它递归进 tool_result.content。core 571 测试绿。计划:`docs/superpowers/plans/2026-06-03-view-image-tool.md`。相关记忆 [[project_todo_items_often_predone]]。**剩下未做(全非阻塞,见下「view_image 收尾」)**。
- [ ] **view_image 收尾(剩 2 个计划明确排除的低优先增量)** —— M1 防御默认、gpt-4o vision 误判已修(见下「已完成」)。剩下两项均为**计划排除**:
...
- [ ] **render 测试用 30ms `setTimeout` 替掉确定性 `flush`(测试坏味道,非阻塞,🟢 最低)** —— **2026-06-03 复核**:原点名的 `render-diff.test.ts` **已不再有此问题**——它现在 `import { mount, flush } from "./render-fixtures"` 用共享 `flush`(`new Promise(r => setImmediate(r))`,确定性排空渲染队列)。但同样的 `function flushRender(){ return new Promise(r => setTimeout(r,30)); }` **墙钟傻等**仍残留在 **`render-scroll.test.ts`**、**`use-virtual-scroll.test.ts`**、**`static-component.test.tsx`** 三个文件。**为什么是问题**:`setTimeout(30)` 不是排空队列,现在 30ms > 16ms 帧间隔能盖住所以不真 flake,但渲染变慢(慢 CI / 未来把写帧挪到 >30ms timer)就偶发失败、失败信号又和别的测试不同难查;且与共享 `flush` 不一致。**修法**:这三个文件改回 `import { flush }`;若 `flush` 对某个测试不灵,在 `render-fixtures.ts` 修根因(让 `flush` 真排空渲染队列),全 render 测试统一等待原语。**未动**:🟢 代码债,且需逐文件确认 `setImmediate`-based `flush` 对各自渲染路径生效(避免盲改引入 flake),非现网 bug。
...
- [x] **旧 session hover 显示当天 16:30(回放误盖 replay 时间)** —— **已修(本轮,未提交)**。**现象**:用户报 `s-mpxn0z3g-3a4a04b2` 这类旧会话,hover assistant 消息显示「16:30」=打开会话那一刻,而非历史时间。**根因**:`foldTranscript` 复用 live reducer `applyStreamEvent`,而 assistant 三处时间戳缺失时都 `?? Date.now()` 兜底(`types.ts` stream_request_start `createdAt: Date.now()`、assistant_message `doneAt ?? Date.now()`、turn_complete `turnDoneAt = Date.now()`);回放事件从不带时间戳(`transcript-reader.ts` 把磁盘 `ev.timestamp` 丢弃),于是每条 assistant 都被打上回放时刻。用户消息没这毛病是因 `appendUserMessage` 回放时不传 createdAt→`formatMessageTime(undefined)` 返回 null→footer 不渲染。**上条「消息加时间节点」的设计注释说 replay 不带时间戳,但 assistant 路径实际漏了。** **修法(方案 A,用户选「不伪造、不显示」)**:`applyStreamEvent` 加可注入时钟 `now: MessageClock = Date.now`(live 不变);`foldTranscript` 传 `() => undefined`,回放 assistant 不再有 createdAt/doneAt → footer 不渲染,与回放用户消息对齐。改 `types.ts`/`foldTranscript.ts`,新增 1 个 RED→GREEN 测试。renderer 251 测试全绿;`tsc` 唯一报错在 `automation-host.ts`(in-flight `contentBlocks` 改动,与本次无关)。重建渲染器(`bun run build:renderer`)后生效。**方案 B(透传真实 `ev.timestamp` 显历史时间)留作后续。**
...
**2026-06-03(/goal「只留未完成 bug + 完善所有 bug」一轮,未提交):**

- **TODO 收口核实**:对全部「未完成」项逐一读码核实(子代理并行 verify),结论——多数已被早前某轮悄悄修掉或本就是误报(对齐记忆 [[project_todo_items_often_predone]]):
  - **OpenAI `finish_reason "length"` 续写不触发** → **已修**(`isTruncatedStop()` in `stop-reason.ts` 同时认 `"length"`/`"max_tokens"`;openai 流式捕获真 `finish_reason`;turn-loop 用之续写;有 `openai-stream-stop.test.ts`+`turn-loop-continuation.test.ts` 覆盖)。关单。
  - **`resolveSandboxBackend` 每 turn 重 resolve(P3)** → **已修**(`engine.ts` per-engine `sandboxCache` 按 `${mode}:${cwd}` 缓存 + runtime 路径 `resolveSandbox`;run() 内只解析一次再喂 toolCtx,非 per-turn)。关单。
  - **RunManager approval/input resume 竞态(P2)** → **核实安全**(`RunApprovalBackend` guard-nullify:先判 pending、立即置 null、再 resolve;二次调用命中 guard 返回 false;Promise 只能 resolve 一次)。关单。
  - **聊天标题显示原始 sessionId** → **误报**(回退「新对话」,绝不回退 id;见上待办区已勾)。关单。
- **🟠 gpt-4o 等被误判为非视觉(影响面超出 view_image,真 bug)** → **已修**:`rules.ts` 补 OpenAI 多模态非推理族规则 `^gpt-4o(?:[-.]|$)|^gpt-4(?:\.1|-turbo|-vision)(?:[-.]|$)`(gpt-4o/4o-mini/4-turbo/4-vision/4.1 → `supportsVision:true`,保留 classic `max_tokens` 不当推理模型;不误伤 gpt-3.5-turbo,不遮挡 gpt-5+)。修前这些著名 vision 模型落 `DEFAULT_CAPABILITY`(vision:false)→ 历史图被剥 / Engine.run 拒新图附件。新增 `capabilities/vision-rules.test.ts`(8 个 gpt-4 族 + gpt-3.5 反例 + gpt-5 不被遮挡);顺手更新 `openai-tool-result-image.test.ts` 过时注释(仍用 gpt-5 保推理 wire 形)。落点 `packages/core/src/llm/capabilities/rules.ts`。
- **🟡 M1 view-image vision gate fail-open** → **已修**:`view-image.ts` gate 原 `if (ctx?.llmConfig){…}` 在缺失时跳过直接读图;改为 fail closed——缺 `llmConfig` 即当非视觉(对齐 `DEFAULT_CAPABILITY.supportsVision=false`),绝不把 base64 读进上下文。运行时 `ToolContext.llmConfig` 必有,缺失只发生在测试/异常装配。新增 view-image 测试:缺 llmConfig 返回文字占位 + gpt-4o(vision)正常出图。
- **验证**:core `bun test` **584 pass / 0 fail**(原 571,+13 新断言)、`tsc --noEmit` 绿、`bun run build` 绿;built dist 实测 gpt-4o/mini/turbo/4.1=vision、gpt-3.5-turbo=非视觉、gpt-5 仍走推理(max_completion_tokens)。

**2026-06-03(view_image 工具一轮,9 commit `97ca194`→`87d9b3a`,在 main 未 push):**

- **view_image 工具**(对照 Codex 同名工具,让模型用 vision 看自己生成的图):新建 `view_image({path})`,三道闸门(vision/格式/大小)防上下文污染——看不了的图(非视觉模型/svg 等格式/>5MB)绝不读 base64 进上下文,只回文字占位。打通「工具结果带图片块」全链路:`ToolResult.contentBlocks` 字段 + `BuiltinToolFn` 返回类型扩展 + registry 透传 + turn-loop `toolResultToBlock` 纯函数 + 两路 provider 适配(Anthropic 映进 tool_result.content;OpenAI 因 wire 限制把 image 提升成独立 user image_url 消息)。transcript 只存文字镜像。**顺修最终 review 抓的 Critical**:`stripVisionFromHistory` 递归进 tool_result.content,堵住「vision 模型 view_image 后切非视觉模型→嵌套 base64 漏发/OpenAI 400」。TDD 逐任务 subagent 驱动 + spec/quality 双审 + 最终 review。core `bun test` 571 pass/0 fail。计划 `docs/superpowers/plans/2026-06-03-view-image-tool.md`。剩 4 个非阻塞收尾项(M1 防御默认、gpt-4o vision 误判、TUI 图片、策略 B 降级)见上「view_image 收尾」。
