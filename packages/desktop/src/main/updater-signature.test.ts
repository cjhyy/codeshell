...
import { macSignatureTextLooksAdHoc, releaseUrlForVersion } from "./updater-signature";
...
describe("updater signature helpers", () => {
  test("detects ad-hoc macOS signatures that cannot pass Squirrel update validation", () => {
    ...
    expect(macSignatureTextLooksAdHoc(text)).toBe(true);
  });
  ...
  test("does not flag Developer ID signatures", () => {
    ...
    expect(macSignatureTextLooksAdHoc(text)).toBe(false);
  });
