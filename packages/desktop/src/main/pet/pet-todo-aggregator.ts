 ...
 * mtime-keyed cache (see mtime-session-cache.ts); no new push channel.
 */
...
export function createPetTodoAggregator(
  ...
  const todoCache = createMtimeSessionCache<SessionTodoItem[] | null>(
    sessionsRootDir,
    (dir) => read(dir),
  ...
  return {
    async collect() {
      ...
      for (const candidate of candidates) {
        const todos = await todoCache.get(candidate.agentSessionId);
