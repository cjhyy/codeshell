...
async function fixture() {
  ...
  const createService = () => {
    ...
    return new PanelAppProcessService(
      optionsForHost.call({
        guests: new Map([[11, { ownerWindowId: 1 }]]),
...
describe("Desktop process consent", () => {
  test("Allow and remember survives an app update and Host restart, but not another app or binary", async () => {
    ...
    await app.run(app.createService(), { ...app.owner, revision: "r2" });
    expect(app.decisions).toHaveLength(1);
    await app.run(app.createService(), { ...app.owner, appId: "another-app", revision: "r2" });
  ...
  test("Cancel never creates a persistent approval", async () => {
    ...
    await expect(app.run(app.createService(), { ...app.owner, revision: "r2" })).rejects.toThrow(
      /denied running/,
    );
    expect(app.decisions).toHaveLength(2);
