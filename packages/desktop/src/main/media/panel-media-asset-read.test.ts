...
test("managed asset chunks are discoverable, bounded and reconstruct exact bytes without exposing paths", async () => {
  ...
  // Asset-read discovery must not invoke a developer machine's optional HyperFrames CLI.
  (f.service as any).dependencies.cliPath = join(f.root, "no-hyperframes-cli");
...
test("the Host speech endpoint does not install panel-owned models or accept clone-specific input", async () => {
  const f = await fixture();
  await expect(
    f.service.dispatch(f.scope, "media.tts.setup", { providerId: "panel-owned-voice" }),
  ).rejects.toThrow("受支持");
  await expect(
    f.service.dispatch(f.scope, "media.tts", {
      text: "test",
      referenceAssetId: f.asset.id,
      referenceText: "reference",
    }),
  ).rejects.toThrow("不支持");
  const catalog = (await f.service.dispatch(f.scope, "media.tts.voices", {})) as any;
  expect(catalog.models.every((model: any) => !model.supportsVoiceCloning)).toBe(true);
  expect(await f.service.jobs.list(f.scope)).toHaveLength(0);
});
