...
describe("buildPetWorkMap closure reminders", () => {
  ...
  test("a completed item with a reminder shows it as detail (overriding summary)", () => {
    const map = buildPetWorkMap(
      [
        session("done", {
          runState: "terminal",
          terminal: { status: "completed", at: 5_000 },
          summary: "本轮已完成",
        }),
      ],
      [],
      { reminders: new Map([["done", "记得补上单测"]]) },
    );
    expect(findItem(map, "done")?.detail).toBe("记得补上单测");
  });

  test("a follow-up item with a reminder shows it as detail", () => {
    const map = buildPetWorkMap(
      [
        session("fu", {
          runState: "idle",
      ...
      [],
      { reminders: new Map([["fu", "待确认异常场景"]]) },
    );
    expect(findItem(map, "fu")?.group).toBe("follow-up");
    expect(findItem(map, "fu")?.detail).toBe("待确认异常场景");
  });

  test("a completed item without a reminder keeps its existing detail", () => {
    const map = buildPetWorkMap(
      [
        session("done", {
          runState: "terminal",
      ...
      [],
      { reminders: new Map([["other", "无关提醒"]]) },
    );
    expect(findItem(map, "done")?.detail).toBe("本轮已完成");
  });

  test("an empty-string reminder does not override the existing detail", () => {
    const map = buildPetWorkMap(
      [
        session("done", {
          runState: "terminal",
      ...
      [],
      { reminders: new Map([["done", ""]]) },
    );
    expect(findItem(map, "done")?.detail).toBe("本轮已完成");
  });

  test("a running item with a reminder in the map is NOT changed", () => {
    const map = buildPetWorkMap(
      [session("run", { runState: "running", summary: "正在运行测试" })],
      [],
      { reminders: new Map([["run", "不该出现的提醒"]]) },
    );
    expect(findItem(map, "run")?.group).toBe("running");
    expect(findItem(map, "run")?.detail).toBe("正在运行测试");
  });
});
