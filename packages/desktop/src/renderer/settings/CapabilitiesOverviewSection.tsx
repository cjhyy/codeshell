 ...
 *
 * The Settings page owns the global/project scope picker. This section renders
 * exactly that selected scope's capability workspace, grouped by kind. Keeping
 * scope ownership at the page level prevents two contradictory scope pickers
 * from appearing in one settings screen.
 *
 *   - 用户(全局) scope → two-state Switch, writes the global settings key
 *     via setCapabilityEnabled(scope:"user").
 *   - 项目 scope       → three-state 继承/开/关 segmented control, writes the project's
...
export function capabilityCacheKey(scope: "user" | "project", projectPath: string | null): string {
  return `caps:${scope === "project" ? (projectPath ?? "") : ""}`;
}
...
export function CapabilitiesOverviewSection({
  ...
  // Seed from this exact scope's last-loaded snapshot so a remount renders
  // synchronously without ever showing another project's controls.
  const [caps, setCaps] = useState<CapabilityDescriptor[]>(
    () => cacheGet<CapabilityDescriptor[]>(cacheKey) ?? [],
  );
  const [loading, setLoading] = useState(() => cacheGet(cacheKey) === undefined);
  const [error, setError] = useState<string | null>(null);
  const [savingId, setSavingId] = useState<string | null>(null);
  // Per-scope monotonic tokens: a slow earlier request for one scope cannot
  // invalidate or overwrite a newer request for another scope.
  const loadSeqByCacheKey = useRef(new Map<string, number>());
  ...
  const load = async () => {
    ...
    const seq = (loadSeqByCacheKey.current.get(requestCacheKey) ?? 0) + 1;
    loadSeqByCacheKey.current.set(requestCacheKey, seq);
    setError(null);
    try {
  ...
  useEffect(() => {
    // A scope change must not leave the previous scope's toggles visible under
    // the new heading. Re-seed from the target cache or show its loading state.
    const cached = cacheGet<CapabilityDescriptor[]>(cacheKey);
    setCaps(cached ?? []);
  ...
  const onUserToggle = async (cap: CapabilityDescriptor, next: boolean) => {
    ...
    try {
      ...
      if (activeCacheKeyRef.current === mutationCacheKey) setSavingId(null);
    }
  };

  const onProjectState = async (cap: CapabilityDescriptor, state: "inherit" | "on" | "off") => {
    const mutationCacheKey = cacheKey;
    setSavingId(cap.id);
    setError(null);
    try {
  ...
  const selectedTitle = selectedProject
    ? (projectLabel ?? projectPath ?? t("settingsX.capOverview.projectConfig"))
    : t("settingsX.capOverview.userGlobal");
      ...
      <div className="flex items-start gap-3 rounded-md border bg-card p-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
            {t("settingsX.capOverview.header")}
          </div>
          <h3 className="m-0 text-[0.95rem] font-semibold text-foreground">
            {t("settingsX.capOverview.title")}
          </h3>
          <p className="m-0 text-xs text-muted-foreground">{t("settingsX.capOverview.desc")}</p>
        </div>
      </div>
      {error && <p className="rounded-md bg-status-err/10 p-2 text-sm text-status-err">{error}</p>}

      <div className="min-h-[420px] rounded-md border p-3">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            <span
              className="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-muted text-muted-foreground"
              aria-hidden
            >
          ...
          <div
            ...
            aria-label={t("settingsX.capOverview.statsAria")}
          >
            ...
            {selectedProject && (
              <span>
                {t("settingsX.capOverview.statOverridden", { count: summary.overridden })}
              </span>
            )}
          </div>
        </div>
                    ...
                    return (
                      <div className="flex items-center gap-3 px-2 py-2 text-sm" key={cap.id}>
                        <span
                          ...
                          title={
                            navigable
                              ? t("settingsX.capOverview.openDetail", {
                                  label: kindLabel(t, g.kind),
                                })
                              : undefined
                          }
                        ...
                        >
                          ...
                          <span className="flex shrink-0 flex-wrap items-center gap-1">
                            ...
                            {selectedProject && (
                              <span
                                className={cn(
                                  "inline-flex items-center rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground",
                                  overridden && "text-primary",
                                )}
                              >
                        ...
                        {scope === "user" ? (
                          ...
                          <div
                            ...
                            aria-label={t("settingsX.capOverview.projectOverrideAria", {
                              name: cap.name,
                            })}
                          >
