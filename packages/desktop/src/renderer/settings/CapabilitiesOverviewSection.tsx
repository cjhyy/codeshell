 ...
 *
 * Left: a scope picker (用户 + each project from projects.ts). Right: the
...
type ScopeNode = { kind: "user" } | { kind: "project"; projectPath: string; label: string };
...
export function CapabilitiesOverviewSection({ projects, onNavigateToKind }: Props) {
  ...
  const cwd = node.kind === "project" ? node.projectPath : "";
  ...
  return (
    <section className="mb-6 flex flex-col gap-3">
      ...
      <div className="grid min-h-[420px] grid-cols-1 gap-3 lg:grid-cols-[240px_1fr]">
        <nav
          className="flex min-h-0 flex-col gap-1 rounded-md border p-2"
          aria-label={t("settingsX.capOverview.scopeAria")}
        >
          <Button
            type="button"
            variant="ghost"
          ...
          >
            ...
            <span className="min-w-0 flex flex-col">
              ...
              <span className="truncate text-xs text-muted-foreground">
                {t("settingsX.capOverview.defaultConfig")}
              </span>
            </span>
          </Button>
          <div className="px-2 pb-1 pt-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
            {t("settingsX.capOverview.projectConfig")}
          </div>
          {projects.length === 0 && (
            <div className="px-2 py-3 text-xs text-muted-foreground">
              {t("settingsX.capOverview.noProjects")}
            </div>
          )}
          {projects.map((r) => {
            const label = projectLabel(r);
            const active = node.kind === "project" && node.projectPath === r.path;
            return (
              <Button
                type="button"
                key={r.id}
                ...
                title={r.path}
                onClick={() => setNode({ kind: "project", projectPath: r.path, label })}
              >
                <span
                  className="flex h-7 w-7 shrink-0 items-center justify-center rounded bg-muted text-muted-foreground"
                  aria-hidden
        ...
        <div className="min-h-0 rounded-md border p-3">
          ...
          {loading && (
            <p className="m-0 text-xs text-muted-foreground">
              {t("settingsX.capOverview.loading")}
            </p>
          )}
