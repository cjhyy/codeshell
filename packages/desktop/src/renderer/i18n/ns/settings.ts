  ...
  zh: {
    settingsX: {
      ...
      adv: {
        ...
        gitPathHelp:
          "留空则用系统 PATH 中的 git。安装插件市场需要 git;若 GUI 启动时没继承到 PATH(Windows 常见),点「选择…」挑出 git 可执行文件(如 C:\\Program Files\\Git\\cmd\\git.exe),或直接填写绝对路径。",
  ...
  en: {
    settingsX: {
      ...
      adv: {
        ...
        gitPathHelp:
          "Leave blank to use git from the system PATH. Installing from the plugin marketplace needs git; if the GUI didn't inherit PATH at launch (common on Windows), click \"Pick…\" to locate the git executable (e.g. C:\\Program Files\\Git\\cmd\\git.exe), or enter an absolute path.",
