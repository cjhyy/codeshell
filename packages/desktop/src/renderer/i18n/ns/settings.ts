  ...
  zh: {
    settingsX: {
      ...
      mcp: {
        ...
        credNone: "（不使用）",
        useCredentialHint: "选「凭证」页里存的 token / link。连接时解析成 Authorization: Bearer，密文不进配置。",
        bearerEnvVarLabel: "Bearer Token 环境变量",
        bearerEnvVarHint:
          "填环境变量「名」（不是值）。连接时从系统环境读取，作为 Authorization: Bearer 发送，token 不会存进配置。",
        envHeadersLabel: "环境变量 Headers (Header: 环境变量名)",
        envHeadersHint:
          "左边是 header 名，右边是环境变量「名」；适合 server 要求自定义鉴权 header（非 Bearer）的场景，值同样连接时才读取。",
        cancel: "取消",
        save: "保存",
        add: "添加",
  ...
  en: {
    settingsX: {
      ...
      mcp: {
        ...
        credNone: "(none)",
        useCredentialHint: "Pick a token / link stored on the Credentials page. Resolved to Authorization: Bearer at connect time; the secret never goes into the config.",
        bearerEnvVarLabel: "Bearer Token env var",
        bearerEnvVarHint:
          "Enter the env var NAME (not the value). Read from the system environment at connect time and sent as Authorization: Bearer; the token is never stored in the config.",
        envHeadersLabel: "Env-var Headers (Header: env-var name)",
        envHeadersHint:
          "Left is the header name, right is the env var NAME; good for servers that require a custom auth header (non-Bearer); values are likewise read only at connect time.",
        cancel: "Cancel",
        save: "Save",
        add: "Add",
        toolsHeader: "{name} — tools ({count})",
