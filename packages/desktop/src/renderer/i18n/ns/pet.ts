export const pet = {
  zh: {
    pet: {
      sidebar: {
        label: "Mimi",
        summary: "{pending} 项等你处理；{running} 项执行中",
      },
      settings: {
        open: "打开 Mimi 设置",
        title: "Mimi 设置",
        subtitle: "管理 Mimi 对话、桌面 Pet 与消息入口",
        back: "返回 Mimi",
        personalizationTitle: "Mimi 个性化",
        personalizationDescription:
          "只作用于 Mimi 管理对话，不会改变普通 Work Session 里 Agent 的回复方式。",
        personalizationBack: "返回 Mimi 设置",
        managePersonalization: "设置个性化",
        responseLanguageLabel: "回复语言",
        responseLanguagePlaceholder: "例如：始终使用简体中文",
        userProfileLabel: "你的称呼与背景",
        userProfilePlaceholder: "例如：叫我 Maki；我做后端开发，熟悉技术细节",
        communicationStyleLabel: "Mimi 的语气与风格",
        communicationStylePlaceholder: "例如：亲切、简洁、直接，主动提醒风险，少用客套话",
        customInstructionsLabel: "给 Mimi 的专属要求",
        customInstructionsPlaceholder: "例如：汇报工作时先说结论；任务有明显风险时先跟我确认…",
        personalizationSaveHint: "自动保存，下一次 Mimi 回复时生效。",
        modelTitle: "Mimi 对话模型",
        modelDescription: "只用于 Mimi 的管理对话；长程工作 Session 继续使用自己的默认模型。",
        useAppDefault: "跟随应用默认",
        followingAppDefault: "当前跟随应用默认模型",
        widgetTitle: "桌面 Pet",
        widgetDescription: "显示桌面悬浮 Mimi、工作动态和未读提醒。",
        connectionsTitle: "消息连接",
        connectionsDescription: "管理个人微信、Telegram 等连接到 Mimi 的渠道。",
        manageConnections: "管理消息连接",
        memoryTitle: "记忆与事件档案",
        memoryDescription: "管理 Mimi 的长期记忆，开关自动提取，回看已归档的对话事件。",
        manageMemory: "打开记忆中心",
      },
      overview: {
        title: "Mimi 工作台",
        subtitle: "把未完成、待处理和可优化的工作整理在一起",
        regionLabel: "Mimi 工作台页面",
        close: "关闭 Mimi 概览",
        summaryLabel: "工作概览",
        expandSummary: "展开工作概览",
        collapseSummary: "收起工作概览",
        loading: "正在加载工作状态",
        reconciling: "正在对账",
        retrying: "加载失败，正在重试",
        unfinishedCount: "{count} 项未完成",
        optimizationCount: "{count} 项可优化",
        completedCount: "{count} 项最近完成",
        unfinishedLabel: "未完成",
        optimizationLabel: "可优化",
        completedLabel: "最近完成",
        runningLabel: "进行中",
        pendingLabel: "待决策",
        followUpLabel: "待跟进",
        updatedNow: "刚刚更新",
        updatedMinutes: "{count} 分钟前更新",
        chatFailureFallback: "工作状态仍可查看",
      },
      work: {
        title: "工作收件箱",
        subtitle: "按 Workspace 和结构化状态整理全部工作",
        loading: "正在整理工作收件箱",
        empty: "目前没有工作记录",
        unassignedWorkspace: "未归属 Workspace",
        hidden: "另有 {count} 项较早记录未展开",
        workspaceItemCount: "{count} 项",
        dismissItemAria: "从工作箱移除「{title}」",
        dismissItemHint: "从工作箱隐藏，不会删除原 Session",
        clearCompleted: "清除已完成",
        clearCompletedHint: "隐藏全部已完成工作，不会删除原 Session",
        restoreDismissed: "恢复隐藏项（{count}）",
        archiveSession: "归档 Session",
        archiveSessionAria: "归档工作会话「{title}」",
        archiveSessionHint: "从工作箱归档，数据仍会保留",
        archiveSessionError: "归档失败，请稍后再试",
        externalUnavailable: "外部会话定位信息不完整或 transcript 已不存在，无法打开",
        latestResultExpandAria: "展开「{title}」的最新结果",
        latestResultCollapseAria: "收起「{title}」的最新结果",
        latestResultLoading: "加载中…",
        latestResultEmpty: "暂无最新结果",
        latestResultError: "无法读取最新结果",
        latestResultTruncated: "…（已截断，打开会话查看全文）",
        branch: {
          running: "进行中",
          pending: "待决策",
          completed: "已完成",
          other: "其他",
        },
        state: {
          "needs-action": "待你处理",
          running: "进行中",
          queued: "排队中",
          failed: "失败待处理",
          cancelled: "已取消",
          completed: "已完成",
          idle: "未分类",
        },
      },
      followUp: {
        title: "需要跟进",
        openSessionAria: "打开会话「{title}」",
        dismissAria: "忽略「{title}」的跟进提醒",
        continue: "继续处理",
        askMimi: "交给 Mimi",
        complete: "已处理",
        dismiss: "忽略",
        askMimiPrompt:
          "请处理「需要跟进」里的这一项：\n跟进 ID：{followUpId}\n来源 Session：{title}\n跟进内容：{text}\n请先用 FollowUps get 核对这个精确 ID，优先接续原 Work Session 执行，完成后再将跟进项标记为已处理。",
      },
      memory: {
        title: "Mimi 记忆",
        summary: "记住了 {count} 条信息",
        empty: "还没有记忆；直接告诉 Mimi「记住…」，或在这里手动添加",
        placeholder: "添加一条 Mimi 应该长期记住的信息…",
        add: "记住",
        save: "保存",
        cancel: "取消",
        edit: "编辑",
        delete: "删除",
        deleteTitle: "删除这条记忆？",
        deleteConfirm: "「{text}」将从 Mimi 的记忆中永久删除。",
        sourceMimi: "Mimi 记录",
        sourceUser: "手动添加",
        sourceAuto: "自动提取",
      },
      memoryCenter: {
        title: "Mimi 记忆中心",
        subtitle: "管理长期记忆，回看已归档的对话事件",
        back: "返回",
        tabMemories: "长期记忆",
        tabJournal: "事件档案",
        autoExtractTitle: "自动提取记忆",
        autoExtractDescription:
          "一段对话结束后，让 Mimi 自动提炼值得长期记住的事实，并整理成事件档案。",
        journalEmpty: "还没有归档的对话事件",
        journalMessageCount: "{count} 条消息",
        viewTranscript: "查看原文",
        hideTranscript: "收起原文",
        transcriptLoading: "正在加载原文…",
        transcriptEmpty: "这段对话没有可显示的原文",
        speakerUser: "我",
        speakerMimi: "Mimi",
      },
      longTask: {
        title: "长程任务",
        activeSummary: "{count} 项正在持续推进",
        noActive: "当前没有进行中的长程任务",
        attempt: "第 {count} 次执行",
        nextAction: "下一步：",
        artifacts: "任务产物",
        openSession: "打开工作会话",
        history: "事件记录",
        expandResult: "展开完整结果",
        collapseResult: "收起结果",
        clearTerminal: "清理已结束（{count}）",
        clearTerminalTitle: "清理已结束任务？",
        clearTerminalConfirm:
          "将永久清理 {count} 条已完成、已取消或失败的长程任务记录。对应的工作会话不会被删除。",
        clearTask: "清理记录",
        clearTaskAria: "清理任务记录：{title}",
        clearTaskTitle: "清理这条任务记录？",
        clearTaskConfirm: "这条已结束的长程任务记录将被永久清理。对应的工作会话不会被删除。",
        clearAction: "清理",
        cancelTaskTitle: "取消这个任务？",
        cancelTaskConfirm: "「{title}」将停止执行。已产生的会话内容会保留，之后仍可重新发起。",
        cancelAction: "取消任务",
        status: {
          queued: "排队中",
          running: "执行中",
          waiting: "等你处理",
          paused: "已暂停",
          interrupted: "可恢复",
          completed: "已完成",
          failed: "执行失败",
          cancelled: "已取消",
        },
        phase: {
          planning: "准备任务",
          executing: "持续执行",
          "waiting-user": "等待用户",
          "waiting-worker": "等待恢复",
          finalizing: "整理结果",
        },
        action: {
          pause: "暂停",
          resume: "继续",
          retry: "重试",
          cancel: "取消",
        },
        event: {
          created: "任务已创建",
          started: "开始执行",
          "run-prepared": "准备继续执行",
          "run-bound": "本轮开始执行",
          "result-updated": "任务结果已更新",
          progress: "进度更新",
          waiting: "等待处理",
          paused: "任务暂停",
          resumed: "恢复执行",
          retrying: "开始重试",
          artifact: "记录任务产物",
          checkpoint: "保存检查点",
          "verification-changed": "完成判定方式已切换",
          "completion-target-set": "已登记完成通知渠道",
          interrupted: "执行中断",
          completed: "任务完成",
          failed: "任务失败",
          cancelled: "任务取消",
          "closure-decided": "Mimi 已记录收口决定",
          "continuation-started": "后续任务已启动",
          "closure-recorded": "任务结果已送达",
          "work-memory-recorded": "结果已记入工作记忆",
        },
      },
      session: {
        title: "工作会话",
        untitled: "未命名会话",
        stateAria: "状态：{state}",
        state: {
          waiting: "等待决策",
          running: "运行中",
          queued: "排队中",
          idle: "空闲",
          dormant: "休眠",
          terminal: "已结束",
          unknown: "状态未知",
        },
        empty: {
          loading: "正在加载会话",
          empty: "还没有工作会话",
          reclaimed: "没有实时工作，worker 已回收",
          disconnected: "worker 已断开，实时状态未知",
          stale: "状态可能已过期",
          error: "暂时无法加载会话，正在自动重试",
          reconciling: "正在对账，会话列表稍后更新",
        },
        justNow: "刚刚",
        minutesAgo: "{count} 分钟前",
        hoursAgo: "{count} 小时前",
        daysAgo: "{count} 天前",
        externalNoNav: "外部会话（由 Codex/Claude CLI 管理），暂不支持在 CodeShell 内打开",
        risk: {
          high: "高风险",
          medium: "中风险",
          low: "低风险",
        },
      },
      pending: {
        title: "待你决定",
        empty: "没有待处理决策",
        open: "打开并处理",
        toolApproval: "工具审批",
        askUser: "需要回答",
      },
      navigation: {
        stale: "该请求已处理或状态已更新；已打开原会话",
        notFound: "原会话已不存在或不可打开",
        externalUnavailable: "外部会话定位信息不完整，无法打开",
      },
      chat: {
        retrySend: "重新发送",
        restoreDraft: "恢复到输入框",
        latestMessages: "回到最新消息",
        stop: "停止回复",
        stopping: "正在停止…",
        stopped: "本轮回复已停止。",
        stopUnavailable: "当前没有可停止的 Mimi 回复。",
        workSummary: "工作动态：{running} 项运行中 · {pending} 项待决定",
        showWork: "展开",
        hideWork: "收起",
        title: "和 Mimi 对话",
        subtitle: "聊想法、理清下一步，需要执行时会自动创建 Session",
        loading: "正在恢复 Mimi 对话",
        failed: "Mimi 对话暂时不可用",
        globalStatus: "全局状态",
        listPending: "列出待决策",
        managerTitle: "Mimi",
        managerBoundary:
          "Mimi 会自动判断：澄清和汇总留在这里，需要读取项目、改代码或运行命令时自动新建 Session。",
        empty: "直接告诉 Mimi 你的问题或目标；需要执行时会自动派发，不用手动选择。",
        emptyTitle: "今天想先处理什么？",
        activity: {
          understanding: "正在理解你的请求…",
          thinking: "正在分析并规划下一步…",
          replying: "正在撰写回复…",
          continuingAfterStep: "已完成：{step}。正在继续处理…",
          adjustingAfterStep: "{step}未成功，正在调整下一步…",
          delegatingSession: "正在派发 Session…",
          delegatingSessionDetail: "正在派发 Session：{detail}…",
          resumingSession: "正在接续 Session…",
          resumingSessionDetail: "正在接续 Session：{detail}…",
          checkingSessions: "正在查看最近的 Session…",
          searchingSessionsDetail: "正在搜索 Session：{detail}…",
          readingSession: "正在读取 Session 的最新进展…",
          checkingFollowUps: "正在查看待跟进事项…",
          searchingFollowUpsDetail: "正在搜索待跟进事项：{detail}…",
          readingFollowUp: "正在读取待跟进事项…",
          updatingFollowUp: "正在更新待跟进事项…",
          searchingChannels: "正在查找可用消息渠道…",
          checkingChannel: "正在确认消息渠道能力…",
          replyingToChannel: "正在回复当前消息会话…",
          sendingMessage: "正在发送消息…",
          updatingMemory: "正在更新长期记忆…",
          controllingLongTask: "正在更新长任务状态…",
          watchingSession: "正在订阅 Session 完成通知…",
          archivingSessions: "正在归档 Session…",
          openingMobileRemote: "正在开启手机远程入口…",
          closingMobileRemote: "正在关闭手机远程入口…",
          checkingTime: "正在确认当前时间…",
          runningTool: "正在执行 {tool}…",
          unknownTool: "当前步骤",
          step: {
            sessionDelegation: "Session 派发",
            sessionDelegationDetail: "Session 派发：{detail}",
            sessionLookup: "Session 查询",
            sessionSearchDetail: "Session 搜索：{detail}",
            followUpLookup: "待跟进事项查询",
            followUpSearchDetail: "待跟进事项搜索：{detail}",
            followUpUpdate: "待跟进事项更新",
            channelLookup: "消息渠道查询",
            channelReply: "消息会话回复",
            messageSend: "消息发送",
            memoryUpdate: "长期记忆更新",
            longTaskControl: "长任务状态更新",
            sessionWatch: "Session 完成通知订阅",
            sessionArchive: "Session 归档",
            mobileRemote: "手机远程入口更新",
            timeLookup: "当前时间查询",
          },
        },
        autoRoute: "自动判断直接回复，或在合适的 Workspace 新建 Session 执行。",
        placeholder: "描述问题、目标或需要跟进的工作…",
        dropFiles: "松开以添加本地文件路径",
        localFilePaths: "本地文件路径（由你拖入）",
        filePathUnavailable: "无法获取该文件的本地路径，请重启应用后重试。",
        removeFile: "移除文件 {name}",
        send: "发送",
        inputQueued: "已排队，将在下一个执行步骤加入当前回复",
        segmentDivider: "新话题",
        workMemoryTitle: "上一段纪要",
        historyCompacted: "上下文已整理，聊天记录保留",
        loadOlderHistory: "加载更早记录",
        loadingOlderHistory: "正在加载更早记录…",
        deliverySent: "已发送到{channel}",
        resultUpdated: "结果已更新",
        openFullResult: "查看完整结果",
        delegation: {
          dispatched: "已派出 Session",
          resumed: "已接续 Session",
          open: "打开 Session",
          openAria: "打开已派出的 Session：{title}",
          state: {
            dispatched: "已派出",
            waiting: "等你处理",
            queued: "排队中",
            running: "执行中",
            started: "已启动",
            completed: "已完成",
            failed: "执行失败",
            cancelled: "已取消",
          },
        },
      },
      peek: {
        close: "关闭提醒",
        openPending: "查看全部待决策",
      },
      widget: {
        open: "打开 Mimi",
        show: "显示 Mimi",
        hide: "隐藏 Mimi",
        miniTitle: "Mimi",
        openFull: "完整页面",
        expandChat: "展开 Mimi 聊天记录",
        collapseChat: "收起 Mimi 聊天记录",
        expandSessions: "展开 Session 动态",
        collapseSessions: "收起 Session 动态",
        placeholder: "问问 Mimi…",
        send: "发送",
        replying: "Mimi 正在回复…",
        replyFallback: "已经处理完成，可打开完整页面查看详情。",
        dragHint: "拖动移动；单击打开聊天；双击打开完整页面；右键菜单可关闭宠物",
        workSummary: "{activity} 项工作提醒；其中 {completed} 项完成未读；{running} 项执行中",
        activityBubbles: "当前工作动态",
        previewAria: "打开 Session：{title}",
        workState: {
          working: "执行中",
          "needs-action": "等你处理",
          completed: "完成未读",
        },
      },
    },
  },
  en: {
    pet: {
      sidebar: {
        label: "Mimi",
        summary: "{pending} need you; {running} in progress",
      },
      settings: {
        open: "Open Mimi settings",
        title: "Mimi settings",
        subtitle: "Manage Mimi chat, the desktop Pet, and messaging entry points",
        back: "Back to Mimi",
        personalizationTitle: "Mimi personalization",
        personalizationDescription:
          "Applies only to Mimi's manager chat and does not change how agents respond in regular Work Sessions.",
        personalizationBack: "Back to Mimi settings",
        managePersonalization: "Customize Mimi",
        responseLanguageLabel: "Reply language",
        responseLanguagePlaceholder: "For example: Always reply in English",
        userProfileLabel: "How to address you & your background",
        userProfilePlaceholder:
          "For example: Call me Maki; I am a backend engineer and am comfortable with technical detail",
        communicationStyleLabel: "Mimi's tone & style",
        communicationStylePlaceholder:
          "For example: Warm, concise, and direct; proactively flag risks and avoid filler",
        customInstructionsLabel: "Mimi-only instructions",
        customInstructionsPlaceholder:
          "For example: Lead work updates with the outcome; ask before proceeding when a task has material risk…",
        personalizationSaveHint: "Saved automatically and applied to Mimi's next reply.",
        modelTitle: "Mimi chat model",
        modelDescription:
          "Used only for Mimi's manager chat. Long-running work Sessions keep their own default model.",
        useAppDefault: "Use app default",
        followingAppDefault: "Currently following the app default model",
        widgetTitle: "Desktop Pet",
        widgetDescription: "Show the floating Mimi, work activity, and unread alerts on desktop.",
        connectionsTitle: "Messaging connections",
        connectionsDescription:
          "Manage Personal WeChat, Telegram, and other channels connected to Mimi.",
        manageConnections: "Manage connections",
        memoryTitle: "Memory & event journal",
        memoryDescription:
          "Manage Mimi's durable memory, toggle auto-extraction, and revisit archived conversation events.",
        manageMemory: "Open memory center",
      },
      overview: {
        title: "Mimi workspace",
        subtitle: "Unfinished, actionable, and improvable work in one place",
        regionLabel: "Mimi workspace page",
        close: "Close Mimi overview",
        summaryLabel: "Work summary",
        expandSummary: "Expand work summary",
        collapseSummary: "Collapse work summary",
        loading: "Loading work status",
        reconciling: "Reconciling",
        retrying: "Load failed; retrying",
        unfinishedCount: "{count} unfinished",
        optimizationCount: "{count} improvements",
        completedCount: "{count} recently completed",
        unfinishedLabel: "Unfinished",
        optimizationLabel: "Can improve",
        completedLabel: "Recently completed",
        runningLabel: "In progress",
        pendingLabel: "Needs decision",
        followUpLabel: "Follow up",
        updatedNow: "Updated just now",
        updatedMinutes: "Updated {count}m ago",
        chatFailureFallback: "Work status remains available",
      },
      work: {
        title: "Work inbox",
        subtitle: "All work organized by workspace and structured status",
        loading: "Building the work inbox",
        empty: "No work records yet",
        unassignedWorkspace: "Unassigned workspace",
        hidden: "{count} older items are collapsed",
        workspaceItemCount: "{count} items",
        dismissItemAria: "Remove “{title}” from the work inbox",
        dismissItemHint: "Hide from the inbox without deleting the original session",
        clearCompleted: "Clear completed",
        clearCompletedHint: "Hide all completed work without deleting the original sessions",
        restoreDismissed: "Restore hidden ({count})",
        archiveSession: "Archive Session",
        archiveSessionAria: "Archive work session “{title}”",
        archiveSessionHint: "Archive it from the work inbox while keeping its data",
        archiveSessionError: "Could not archive this session; try again",
        externalUnavailable:
          "The external session locator is incomplete or its transcript no longer exists",
        latestResultExpandAria: "Expand the latest result of “{title}”",
        latestResultCollapseAria: "Collapse the latest result of “{title}”",
        latestResultLoading: "Loading…",
        latestResultEmpty: "No result yet",
        latestResultError: "Could not read the latest result",
        latestResultTruncated: "… (truncated — open the session for the full text)",
        branch: {
          running: "In progress",
          pending: "Needs decision",
          completed: "Completed",
          other: "Other",
        },
        state: {
          "needs-action": "Needs you",
          running: "In progress",
          queued: "Queued",
          failed: "Failed",
          cancelled: "Cancelled",
          completed: "Completed",
          idle: "Unclassified",
        },
      },
      followUp: {
        title: "Needs follow-up",
        openSessionAria: "Open session “{title}”",
        dismissAria: "Dismiss the follow-up for “{title}”",
        continue: "Continue",
        askMimi: "Ask Mimi",
        complete: "Handled",
        dismiss: "Dismiss",
        askMimiPrompt:
          "Please handle this item from Needs follow-up:\nFollow-up ID: {followUpId}\nSource Session: {title}\nFollow-up: {text}\nUse FollowUps get to verify this exact ID, resume the original Work Session when possible, then mark the item handled after the work is complete.",
      },
      memory: {
        title: "Mimi memory",
        summary: "{count} things remembered",
        empty: "No memories yet; tell Mimi to “remember …” or add one here",
        placeholder: "Add something Mimi should remember long-term…",
        add: "Remember",
        save: "Save",
        cancel: "Cancel",
        edit: "Edit",
        delete: "Delete",
        deleteTitle: "Delete this memory?",
        deleteConfirm: "“{text}” will be permanently removed from Mimi's memory.",
        sourceMimi: "Saved by Mimi",
        sourceUser: "Added manually",
        sourceAuto: "Auto-extracted",
      },
      memoryCenter: {
        title: "Mimi memory center",
        subtitle: "Manage durable memory and revisit archived conversation events",
        back: "Back",
        tabMemories: "Durable memory",
        tabJournal: "Event journal",
        autoExtractTitle: "Auto-extract memory",
        autoExtractDescription:
          "After a conversation winds down, let Mimi distill durable facts and file the segment as an event.",
        journalEmpty: "No archived conversation events yet",
        journalMessageCount: "{count} messages",
        viewTranscript: "View transcript",
        hideTranscript: "Hide transcript",
        transcriptLoading: "Loading transcript…",
        transcriptEmpty: "No transcript to show for this segment",
        speakerUser: "Me",
        speakerMimi: "Mimi",
      },
      longTask: {
        title: "Long-running tasks",
        activeSummary: "{count} continuing in the background",
        noActive: "No active long-running tasks",
        attempt: "Attempt {count}",
        nextAction: "Next:",
        artifacts: "Task artifacts",
        openSession: "Open work session",
        history: "Event history",
        expandResult: "Expand full result",
        collapseResult: "Collapse result",
        clearTerminal: "Clear ended ({count})",
        clearTerminalTitle: "Clear ended tasks?",
        clearTerminalConfirm:
          "This permanently clears {count} completed, cancelled, or failed long-task records. Their work sessions will not be deleted.",
        clearTask: "Clear record",
        clearTaskAria: "Clear task record: {title}",
        clearTaskTitle: "Clear this task record?",
        clearTaskConfirm:
          "This ended long-task record will be permanently cleared. Its work session will not be deleted.",
        clearAction: "Clear",
        cancelTaskTitle: "Cancel this task?",
        cancelTaskConfirm:
          "“{title}” will stop running. Produced session content is kept and the task can be started again later.",
        cancelAction: "Cancel task",
        status: {
          queued: "Queued",
          running: "Running",
          waiting: "Needs you",
          paused: "Paused",
          interrupted: "Resumable",
          completed: "Completed",
          failed: "Failed",
          cancelled: "Cancelled",
        },
        phase: {
          planning: "Preparing",
          executing: "Executing",
          "waiting-user": "Waiting for user",
          "waiting-worker": "Waiting to resume",
          finalizing: "Finalizing",
        },
        action: {
          pause: "Pause",
          resume: "Resume",
          retry: "Retry",
          cancel: "Cancel",
        },
        event: {
          created: "Task created",
          started: "Execution started",
          "run-prepared": "Preparing to continue",
          "run-bound": "Current turn started",
          "result-updated": "Task result updated",
          progress: "Progress updated",
          waiting: "Waiting for input",
          paused: "Task paused",
          resumed: "Execution resumed",
          retrying: "Retry started",
          artifact: "Artifact recorded",
          checkpoint: "Checkpoint saved",
          "verification-changed": "Completion policy changed",
          "completion-target-set": "Completion notification route registered",
          interrupted: "Execution interrupted",
          completed: "Task completed",
          failed: "Task failed",
          cancelled: "Task cancelled",
          "closure-decided": "Mimi recorded the closure decision",
          "continuation-started": "Continuation started",
          "closure-recorded": "Task outcome delivered",
          "work-memory-recorded": "Outcome recorded in work memory",
        },
      },
      session: {
        title: "Work sessions",
        untitled: "Untitled session",
        stateAria: "Status: {state}",
        state: {
          waiting: "Waiting for a decision",
          running: "Running",
          queued: "Queued",
          idle: "Idle",
          dormant: "Dormant",
          terminal: "Finished",
          unknown: "Status unknown",
        },
        empty: {
          loading: "Loading sessions",
          empty: "No work sessions yet",
          reclaimed: "No live work; worker reclaimed",
          disconnected: "Worker disconnected; live status unknown",
          stale: "Status may be stale",
          error: "Sessions are temporarily unavailable; retrying",
          reconciling: "Reconciling; sessions will update shortly",
        },
        externalNoNav:
          "External session (managed by the Codex/Claude CLI); cannot be opened inside CodeShell",
        risk: {
          high: "High risk",
          medium: "Medium risk",
          low: "Low risk",
        },
        justNow: "Just now",
        minutesAgo: "{count}m ago",
        hoursAgo: "{count}h ago",
        daysAgo: "{count}d ago",
      },
      pending: {
        title: "Waiting for you",
        empty: "No pending decisions",
        open: "Open and handle",
        toolApproval: "Tool approval",
        askUser: "Answer needed",
      },
      navigation: {
        stale: "The request changed or was handled; opened the original session",
        notFound: "The original session no longer exists or cannot be opened",
        externalUnavailable: "The external session locator is incomplete and cannot be opened",
      },
      chat: {
        retrySend: "Retry send",
        restoreDraft: "Restore draft",
        latestMessages: "Latest messages",
        stop: "Stop reply",
        stopping: "Stopping…",
        stopped: "This reply was stopped.",
        stopUnavailable: "There is no active Mimi reply to stop.",
        workSummary: "Work: {running} running · {pending} decisions",
        showWork: "Show",
        hideWork: "Hide",
        title: "Chat with Mimi",
        subtitle: "Talk through ideas and next steps; execution starts a Session automatically",
        loading: "Restoring Mimi chat",
        failed: "Mimi chat is temporarily unavailable",
        globalStatus: "Global status",
        listPending: "List pending",
        managerTitle: "Mimi",
        managerBoundary:
          "Mimi routes automatically: clarification and summaries stay here; project inspection, edits, and commands start a new session.",
        empty:
          "Tell Mimi the question or goal. Work that needs execution is delegated automatically.",
        emptyTitle: "What should we tackle first?",
        activity: {
          understanding: "Understanding your request…",
          thinking: "Analyzing and planning the next step…",
          replying: "Writing a reply…",
          continuingAfterStep: "Finished: {step}. Continuing…",
          adjustingAfterStep: "{step} did not succeed. Adjusting the next step…",
          delegatingSession: "Dispatching a Session…",
          delegatingSessionDetail: "Dispatching a Session: {detail}…",
          resumingSession: "Resuming a Session…",
          resumingSessionDetail: "Resuming a Session: {detail}…",
          checkingSessions: "Checking recent Sessions…",
          searchingSessionsDetail: "Searching Sessions: {detail}…",
          readingSession: "Reading the latest Session progress…",
          checkingFollowUps: "Checking follow-ups…",
          searchingFollowUpsDetail: "Searching follow-ups: {detail}…",
          readingFollowUp: "Reading a follow-up…",
          updatingFollowUp: "Updating a follow-up…",
          searchingChannels: "Finding available message channels…",
          checkingChannel: "Checking message channel capabilities…",
          replyingToChannel: "Replying to the current message thread…",
          sendingMessage: "Sending a message…",
          updatingMemory: "Updating long-term memory…",
          controllingLongTask: "Updating a long-running task…",
          watchingSession: "Subscribing to Session completion…",
          archivingSessions: "Archiving Sessions…",
          openingMobileRemote: "Opening mobile remote access…",
          closingMobileRemote: "Closing mobile remote access…",
          checkingTime: "Checking the current time…",
          runningTool: "Running {tool}…",
          unknownTool: "current step",
          step: {
            sessionDelegation: "Session dispatch",
            sessionDelegationDetail: "Session dispatch: {detail}",
            sessionLookup: "Session lookup",
            sessionSearchDetail: "Session search: {detail}",
            followUpLookup: "follow-up lookup",
            followUpSearchDetail: "follow-up search: {detail}",
            followUpUpdate: "follow-up update",
            channelLookup: "message channel lookup",
            channelReply: "message thread reply",
            messageSend: "message send",
            memoryUpdate: "long-term memory update",
            longTaskControl: "long-running task update",
            sessionWatch: "Session completion subscription",
            sessionArchive: "Session archive",
            mobileRemote: "mobile remote access update",
            timeLookup: "current time lookup",
          },
        },
        autoRoute:
          "Mimi replies directly or starts a session in the appropriate workspace automatically.",
        placeholder: "Describe a problem, goal, or follow-up…",
        dropFiles: "Drop to add local file paths",
        localFilePaths: "Local file paths (dropped by you)",
        filePathUnavailable:
          "Could not resolve this file's local path. Restart the app and try again.",
        removeFile: "Remove file {name}",
        send: "Send",
        inputQueued: "Queued to join the current reply at the next step",
        segmentDivider: "New topic",
        workMemoryTitle: "Previous segment recap",
        historyCompacted: "Context compacted; chat history is preserved",
        loadOlderHistory: "Load earlier messages",
        loadingOlderHistory: "Loading earlier messages…",
        deliverySent: "Sent to {channel}",
        resultUpdated: "Result updated",
        openFullResult: "View full result",
        delegation: {
          dispatched: "Session dispatched",
          resumed: "Session resumed",
          open: "Open Session",
          openAria: "Open dispatched Session: {title}",
          state: {
            dispatched: "Dispatched",
            waiting: "Needs you",
            queued: "Queued",
            running: "Running",
            started: "Started",
            completed: "Completed",
            failed: "Failed",
            cancelled: "Cancelled",
          },
        },
      },
      peek: {
        close: "Dismiss notification",
        openPending: "View all pending",
      },
      widget: {
        open: "Open Mimi",
        show: "Show Mimi",
        hide: "Hide Mimi",
        miniTitle: "Mimi",
        openFull: "Open full view",
        expandChat: "Expand Mimi chat history",
        collapseChat: "Collapse Mimi chat history",
        expandSessions: "Expand Session activity",
        collapseSessions: "Collapse Session activity",
        placeholder: "Ask Mimi…",
        send: "Send",
        replying: "Mimi is replying…",
        replyFallback: "Done. Open the full view for details.",
        dragHint:
          "Drag to move; click for chat; double-click for the full view; right-click to close the Pet",
        workSummary: "{activity} work alerts; {completed} completed unread; {running} in progress",
        activityBubbles: "Current work activity",
        previewAria: "Open Session: {title}",
        workState: {
          working: "In progress",
          "needs-action": "Needs you",
          completed: "Completed unread",
        },
      },
    },
  },
} as const;
