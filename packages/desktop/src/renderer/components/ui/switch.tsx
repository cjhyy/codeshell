...
>(({ className, ...props }, ref) => (
  <SwitchPrimitives.Root
    className={cn(
      "cs-switch peer inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border border-transparent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-50",
      className,
    )}
    {...props}
    ref={ref}
  >
    <SwitchPrimitives.Thumb className="cs-switch-thumb pointer-events-none block h-4 w-4 rounded-full shadow-sm ring-0 transition-transform" />
  </SwitchPrimitives.Root>
));
