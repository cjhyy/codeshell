...
import type { ArenaMode, ArenaParticipant } from "./types.js";
  ...
  inputSchema: {
    ...
    properties: {
      ...
      mode: {
        ...
        description:
          "Arena mode override. Usually auto-detected from topic. 'review' for structured review, 'discussion' for open analysis, 'planning' for roadmaps.",
      },
      participants: {
        type: "array",
        items: { type: "string" },
      ...
      concluder: {
        type: "string",
        description:
          "Which participant builds the final consensus (by name). Default: first participant.",
      },
    },
    required: ["topic"],
  },
...
export function getArenaStatus(): ArenaStatus {
  ...
  const defaults =
    fromSettings.length >= 2 ? fromSettings : resolveDefaultParticipantNames(entries);
...
function participantWouldResolve(
  ...
  return probeParticipant(nameOrPath, poolEntries, baseUrl ? ({ baseUrl } as LLMConfig) : undefined)
    .compatible;
}
...
function readSettingsParticipants(cwd?: string): string[] {
  ...
  try {
    const settings = new SettingsManager(cwd ?? process.cwd(), "full").get() as unknown as {
      capabilities?: { arena?: { participants?: unknown[] } };
      arena?: { participants?: unknown[] };
    };
...
export function saveArenaSettingsByKeys(keys: string[]): void {
  ...
  new SettingsManager(process.cwd(), "full").saveUserSetting(
    "capabilities.arena.participants",
    participants,
  );
}
...
export async function arenaTool(args: Record<string, unknown>, ctx?: ToolContext): Promise<string> {
  ...
  try {
    ...
    const progressSummary =
      progressLog.length > 0 ? "── Arena Progress ──\n" + progressLog.join("\n") + "\n\n" : "";
