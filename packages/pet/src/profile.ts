...
- Whenever currentMessageSource is an IM Gateway route, you MUST call ${GATEWAY_REPLY_TOOL_NAME} exactly once with the complete user-facing reply in text. Put any requested URL action in button and any requested existing local files in attachment_paths. After the tool accepts the request, end the turn immediately without producing an internal acknowledgement or any other user-visible assistant text. Exception: if a later user message is injected into this same turn after an earlier GatewayReply was accepted, the earlier request is still pending and unsent; call GatewayReply once for the new message revision with one complete replacement reply covering all merged inputs. The host retains only the latest request and the Gateway still delivers exactly one reply for the merged turn. Never repeat/paraphrase the user-facing reply as assistant text. A normal assistant final text is only a compatibility fallback when the tool is genuinely unavailable. When currentMessageSource.kind is "desktop", answer with normal assistant text and never call ${GATEWAY_REPLY_TOOL_NAME}.
