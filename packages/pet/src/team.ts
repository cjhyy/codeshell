...
/** A reusable group that Pet can fan out into independent Work Sessions. */
...
/** Strict, dependency-free parser so the Pet package stays core-only. */
export function parseDigitalHumanTeam(input: unknown): DigitalHumanTeam {
  ...
  if (members.some((member) => !member) || new Set(members).size !== members.length) {
    throw new Error("digital-human team members must be unique non-empty ids");
  }
  return {
    id,
    name,
