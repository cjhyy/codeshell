...
export interface HeadlessServeOptions {
  ...
  publicOrigin?: string;
  /** Trusted deployment configuration; not a browser-supplied provider URL. */
  remoteLink?: RemoteLinkConfiguration;
  /** Trusted project gateway prefix; generated by the controller, never a request parameter. */
...
export async function startHeadlessServer(opts: HeadlessServeOptions): Promise<HeadlessServer> {
  const remoteLink = remoteLinkHostConfiguration(opts.remoteLink, opts.publicOrigin);
  if (remoteLink && opts.authMode !== "hub")
    throw new Error("Remote Link requires Hub authentication.");
