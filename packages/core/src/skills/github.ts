 ...
 *   3. installFromGithub: download the bounded tree at the reviewed commit, find
...
const dlog = (_scope: string, event: string, data: Record<string, unknown>) =>
...
export interface RepoInspection {
  ...
  /** Immutable revision reviewed by the user. All preview/downloads use this commit. */
  commit?: string;
...
export function parseSkillSourceMeta(value: unknown): SkillSourceMeta | null {
  ...
  if (
    ...
    (raw.dirInRepo !== "" && !safeRepoPath(raw.dirInRepo)) ||
...
async function getRepoTree(info: GithubUrlInfo, ref: string): Promise<TreeResponse> {
 ...
 */
export async function getRefCommit(info: GithubUrlInfo, ref: string): Promise<string> {
  assertGithubInfo(info, ref);
  ...
  try {
    const res = await fetch(
      `https://raw.githubusercontent.com/${encodeURIComponent(info.owner)}/${encodeURIComponent(info.repo)}/${encodeURIComponent(ref)}/${pathInRepo
        ...
        .join("/")}`,
      { headers: { "User-Agent": USER_AGENT }, signal: controller.signal, redirect: "error" },
    );
    if (!res.ok) throw new Error(`raw fetch ${res.status} ${res.statusText}`);
    return (
      await readBoundedResponse(res, MAX_FRONTMATTER_FILE_BYTES, "SKILL.md preview")
    ).toString("utf8");
  } finally {
    clearTimeout(t);
  }
...
export async function inspectRepo(
  ...
  if (
    ...
    !meta.default_branch ||
    meta.default_branch.length > 512
  ) {
    throw new Error("GitHub repository response has an invalid default branch");
  }
  const ref = info.ref || meta.default_branch;
  const commit = await getRefCommit(info, ref);
  const tree = await getRepoTree(info, commit);
  ...
  const skillBlobs = nodes.filter(
    (n) =>
      ...
      (!info.subpath ||
        n.path.startsWith(info.subpath.replace(/\/$/, "") + "/") ||
        n.path === info.subpath),
  );
  ...
  const detailed = await Promise.all(
    subset.map(async (blob) => {
      const dir = blob.path === "SKILL.md" ? "" : blob.path.replace(/\/SKILL\.md$/, "");
      let parsed: ParsedFrontmatter = {};
      try {
        const text = await getRawFile(info, commit, blob.path);
      ...
      const folderName = dir.split("/").pop() || info.repo;
...
export async function downloadSkillTree(
  ...
  assertGithubInfo(info, ref);
  if ((dirInRepo !== "" && !safeRepoPath(dirInRepo)) || dirInRepo.length > 4_096) {
    throw new Error("invalid skill directory path");
  }
  const tree = await getRepoTree(info, ref);
  if (!tree || !Array.isArray(tree.tree)) throw new Error("GitHub tree response is invalid");
  if (tree.truncated) throw new Error("GitHub 未返回完整目录树，请选择更小的 Skill 仓库。");
  const nodes = tree.tree.filter(isTreeNode);
  const prefix = dirInRepo ? dirInRepo.replace(/\/$/, "") + "/" : "";
  ...
  for (const f of files) {
    if (f.mode === "120000") throw new Error("Skill 包含符号链接，不能安全导入。");
    ...
    try {
      ...
      await fs.writeFile(localPath, buf, { mode: f.mode === "100755" ? 0o700 : 0o600 });
    } finally {
      clearTimeout(t);
    }
  }
...
export async function installFromGithub(input: InstallFromGithubInput): Promise<InstalledSkill> {
  ...
  const commit = inspection.commit || (await getRefCommit(inspection.url, ref));
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "codeshell-gh-skill-"));
  try {
    await downloadSkillTree(inspection.url, commit, reviewedSelection.dirInRepo, tmpRoot);
    try {
      await fs.access(path.join(tmpRoot, "SKILL.md"));
    } catch {
      throw new Error(`下载结果缺少 SKILL.md：${reviewedSelection.dirInRepo}`);
    }
    // Stage provenance with the bundle, so readers never observe a completed
    // installation without the source revision needed for future updates.
    ...
    await fs.writeFile(path.join(tmpRoot, SKILL_META_FILE), JSON.stringify(meta, null, 2), {
      mode: 0o600,
    });
    return installSkillFromDirectory(tmpRoot, scope, cwd, installName || reviewedSelection.name);
  } finally {
 ...
 */
export async function checkSkillUpdate(filePath: string): Promise<SkillUpdateCheck> {
  ...
  let latest: string;
  try {
    latest = await getRefCommit({ owner: meta.owner, repo: meta.repo, ref: meta.ref }, meta.ref);
  } catch (e) {
    return {
      filePath,
...
export async function updateSkillFromSource(
  ...
  assertOwnedSkillDirectory(filePath, [root]);
  const previous = readSkillBundle(dir);
  if (readBoundedSkillFile(metaPath, MAX_SOURCE_META_BYTES).toString("utf8") !== raw)
  ...
  try {
    await deps.downloadSkillTree(info, latest, meta.dirInRepo, tmpRoot);
    try {
      await fs.access(path.join(tmpRoot, "SKILL.md"));
    } catch {
    ...
    await fs.writeFile(path.join(tmpRoot, SKILL_META_FILE), JSON.stringify(nextMeta, null, 2), {
      mode: 0o600,
    });
    ...
    try {
      ...
      commitSkillDirectory(stage, root, path.basename(dir), previous.revision);
    } catch (error) {
      throw new Error(
        `更新失败，已保留(restored/kept)旧版本：${error instanceof Error ? error.message : String(error)}`,
        { cause: error },
      );
    } finally {
