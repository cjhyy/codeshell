...
describe("CredentialStore", () => {
  ...
  test("oauth credential round-trips as a first-class credential type", () => {
    const store = new CredentialStore(cwd);
    ...
    store.save("user", {
      ...
      secret,
      meta: {
        appUrl: "https://www.figma.com/oauth",
        oauthProvider: "figma",
        tokenEndpoint: "https://auth.example.com/token",
        clientId: "client-abc",
        scopes: ["files:read"],
    ...
    const c = store.resolve("figma-oauth")!;
    expect(c.type).toBe("oauth");
    expect(JSON.parse(c.secret!).accessToken).toBe("access-123");
    expect(c.meta?.oauthProvider).toBe("figma");
