...
function claudeResponse(headers: Record<string, string>, ok = true, status = 200): typeof fetch {
  ...
  return (async () => ({ ok, status, headers: h }) as unknown as Response) as unknown as typeof fetch;
}
...
describe("formatQuota", () => {
  ...
  it("renders percent + relative reset", () => {
    const out = formatQuota(
      { codex: { provider: "codex", planType: "team", windows: [{ kind: "5h", usedPercent: 21, resetsAt: 1782893448 }] } },
      now,
    );
