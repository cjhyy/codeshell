...
当前富媒体纵向链路先覆盖个人微信和 Telegram 的入站图片、文件、语音、视频。附件在 sender/
target 白名单通过后才按需下载，受数量、单文件、总大小和超时限制；随后由 Electron main 校验并
暂存到 Pet 会话的 `.code-shell/attachments` 目录。其他 channel 的富媒体映射和媒体回发将按各平台
上传协议逐步补齐，文字收发不受影响。
...
尚未包含 launchd/systemd 安装器；生产使用应由现有进程守护器托管。
