...
describe("CodexThreadContextStore", () => {
  test("resolves a registered thread to its own host", () => {
    ...
    store.register("thread-a", a, 1);
    store.register("thread-b", b, 1);
  ...
  test("fails closed on a missing thread id", () => {
    const store = new CodexThreadContextStore();
    store.register("thread-a", fakeHost("sess-a"), 1);
  ...
  test("generation fencing rejects a late request from a previous app-server", () => {
    ...
    const store = new CodexThreadContextStore();
    store.register("thread-a", fakeHost("sess-old"), 1);
    store.bumpGeneration();
    store.register("thread-a", fakeHost("sess-new"), 2);
  ...
  test("a batch spanning two threads is rejected wholesale", () => {
    ...
    const store = new CodexThreadContextStore();
    store.register("thread-a", fakeHost("sess-a"), 1);
    store.register("thread-b", fakeHost("sess-b"), 1);

    const single = store.resolveBatch(["thread-a", "thread-a"], 1);
    expect(single.ok).toBe(true);

    const mixed = store.resolveBatch(["thread-a", "thread-b"], 1);
  ...
  test("a batch with any missing thread id is rejected", () => {
    const store = new CodexThreadContextStore();
    store.register("thread-a", fakeHost("sess-a"), 1);
    const result = store.resolveBatch(["thread-a", undefined], 1);
    expect(result.ok).toBe(false);
    expect(result.ok === false && result.reason).toBe("missing_thread_id");
  });
  ...
  test("an empty batch is rejected rather than treated as trivially fine", () => {
    ...
    const store = new CodexThreadContextStore();
    store.register("thread-a", fakeHost("sess-a"), 1);
    const result = store.resolveBatch([], 1);
    expect(result.ok).toBe(false);
    expect(result.ok === false && result.reason).toBe("missing_thread_id");
  });
  ...
  test("re-registering a thread replaces the host instead of accumulating", () => {
    ...
    store.register("thread-a", first, 1);
    store.register("thread-a", second, 1);
