...
describe("parseJudgeResponse", () => {
  ...
  it("parses continue-fresh with handoff summary", () => {
    const d = parseJudgeResponse(
      '{"action":"continue-fresh","handoffSummary":"prev built X","reason":"unrelated next step"}',
    );
...
describe("judgeContinuation", () => {
  it("calls the injected aux LLM with goal + lastResult + nextPrompt and returns parsed decision", async () => {
    ...
    const d = await judgeContinuation(
      { goal: "all tests pass", lastResult: "tests green", nextPrompt: "rerun" },
      fakeLlm,
    );
    expect(d.action).toBe("stop");
