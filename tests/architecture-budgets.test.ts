...
describe("architecture growth budgets", () => {
  test("large composition roots cannot grow without an extraction", () => {
    // Exact 2026-08-11 post-audit baselines: decreases are welcome; any growth
    // must extract a module or consciously update this architectural decision.
    expect(lines("packages/desktop/src/main/index.ts")).toBeLessThanOrEqual(6_346);
    ...
    expect(lines("packages/desktop/src/preload/types.d.ts")).toBeLessThanOrEqual(2_828);
    expect(lines("packages/desktop/src/renderer/App.tsx")).toBeLessThanOrEqual(2_686);
    expect(lines("packages/core/src/protocol/server.ts")).toBeLessThanOrEqual(4_442);
    expect(lines("packages/core/src/engine/engine.ts")).toBeLessThanOrEqual(4_204);
  });
  ...
  test("published entry points cannot silently expand their compatibility surface", () => {
    const exportBudgets: Record<string, number> = {
      ...
      "packages/core/src/index.internal.ts": 79,
