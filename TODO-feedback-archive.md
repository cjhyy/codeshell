...
> 从 `TODO-feedback.md` 归档的已修复(🟢)/不修(⚪️)项,保留完整根因与修复笔记备查。
> 主文件只保留未完成(🔴/🟡)项。归档日期:2026-06-16。
...
- **状态**:🟢 v0.1 + validator(2026-06-14;Codex 式校验脚本已补,eval 循环明确不做)
- **已做**:skill-creator v0.1 作为官方市场 **mimi-plugins** 首个插件上线(github cjhyy/mimi-plugins,commit 6d0985f)——五步引导 meta-skill(访谈意图→选位置→按模板起草→校验→触发测试),走 CC 风格交互访谈而非 Codex 脚手架;随首启软预装自动到位(见 #22)。**2026-06-14 补 Codex 式 validator**(mimi-plugins commit 8fbe2c6):新增 `skills/skill-creator/scripts/quick_validate.py`,第 4 步从「逐项肉眼检查」改成跑脚本——确定性校验 frontmatter 可解析 / name kebab-case 且与目录名一致 / description 长度与触发措辞 / 正文非空不过长,退出码 0 过 1 须修;无第三方依赖。SKILL.md 第 4 步改为先跑脚本、仅留「正文步骤可执行」一项肉眼语义检查。**明确不做**:eval/benchmark 循环(CC skill-creator 的 Evaluate/Improve 阶段是给严肃维护的 skill 库用的重型设施,对刚起步的官方市场属过度工程)。
